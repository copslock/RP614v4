...
//KEVIN:05DEC15:ADD:BlankState+++
int Check_BlankState(request* req) 
//return : 0 : pathfile is not Blankstate file
//		 1 : pathfile is Blankstate file
//		-1: 
{
	int CmpResult, SmtWzdState;
	//int index=MAX_PATTERN_NUM;
	//char FilePattern[MAX_PATTERN_NUM][20] = {"", "Blank_", 
	//											"img", "Error", 
	//										"css", "cgi",
	//										"gif","js",
	//										"basicsetting", "CA"};
	...
	//if(req->pathname == 0) { return 1; } //when request is CGI
	
	//if (strstr(req->pathname, "blockurl.html")) return 1;	
	
	SmtWzdState = atoi(nvram_get("SmtWzdDone"));
	
	if (req->pathname == 0 && !SmtWzdState) { return 1; } //when request is CGI
	
	if (req->pathname == 0) {
		if(!strcmp(req->request_uri,"/cgi-bin/CA.cgi")) return 1;
		if(!strcmp(req->request_uri,"/cgi-bin/TraceWanDet.cgi")) return 1;
		return 0;
	}
	if (strstr(req->pathname, "blockurl.html")) return 1;	

	//In normal state, do auth
	if(SmtWzdState) {
		while(FilePattern2[index]) {
			if(strstr(req->pathname, FilePattern2[index])) {
				return 1;		
			}
			index++;	
		}	
		BlankState_httpd_check_auth(NULL); //Reset 
		return 0; //In normal state, do authorize. except Error page.
	}
	
	if(!BlankState_httpd_check_auth(req)) {
		return -1;
	}
#if 0 
	CmpResult = strcmp(req->pathname, "/www/");
	if(!CmpResult) {
	//if (!SmtWzdState) {		
		return 1;
	}
	...
	index=0;
	while (FilePattern[index]) {
		if(strstr(req->pathname, FilePattern[index])) {
			return 1;		
		}
		index++;	
	}	
...
void Set_Flag_WAN_LAN_MATCH_DET(int flag);
#endif
/*
...
extern int Poped_Msg_Subnet_match=1;
...
#ifdef USE_AUTH
				closelog();
				RetVal = Check_BlankState(req);
#ifdef CONFIG_WAN_LAN_MATCH_DETECTION
						{
							int WanLanFlag = atoi(nvram_get("WL_ip_match"));
							if(WanLanFlag) {
								RetVal = 1;
							}
						}
#endif
//				printf("path name is %s, and RetVal is %d\n",
//					req->pathname,
//					RetVal);
				switch(RetVal) {
					case -1:
					...
					default:
						...
						Check_Reload_Cfg(req);
				}
#if 0
				if(!Check_BlankState(req)) { //KEVIN:05DEC15:ADD:BlankState
					openlog("boa", LOG_PID, LOG_AUTHPRIV);
					if (!auth_authorize(req)) {
						openlog("boa", LOG_PID, 0);
						return 0;
...
#if defined(DNI)		
		printf("req->post_file_name = %s\n", req->post_file_name);
		printf("req->request_uri = %s\n", req->request_uri);				
		if (strstr(req->request_uri, "/cgi-bin/upgrade.cgi")) {
			upgrading = 1;
			setLedLight(0/*TEST_LED*/, 0/*LED_ON*/);
			do_kill("/var/run/checkwan.pid");			
			stop_wan();	
			do_kill("/var/run/udhcpc.pid");
			do_kill("/var/run/pppd0.pid");
			do_kill("/var/run/pptp.pid");
			do_kill("/var/run/l2tpd.pid");			
			remove("/var/run/pppd0");	
			remove("/var/run/pptp_redial");
			remove("/var/ppp/chap0");
			remove("/var/ppp/pap0");
			remove("/var/ppp/options0");
			stop_service();			
			//do_kill("/var/run/checkwan.pid");
		}	 		
#endif
	...
	if (req->is_cgi && (req->method == M_POST))
#ifdef SUPPORT_ASP
#if defined(DNI)
