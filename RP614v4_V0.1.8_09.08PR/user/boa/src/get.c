...
int init_get(request * req)
{
	...
	find_first(req, req->pathname);		/* for http proxy */
	data_fd = open(req->pathname, O_RDONLY);
...
#if defined(DNI)
			/* if pages not found then always redirect to welcome page */
			//if (atoi(nvram_get("SmtWzdDone")) == 0) {
				data_fd = get_dir(req, &statbuf);	/* updates statbuf */
				if (data_fd == -1)		/* couldn't do it */
					return 0;			/* errors reported by get_dir */
				else if (data_fd == 0)
...
#if defined(DNI)
    /* if .htm || .asp then need to sht */
    if ((strstr(req->pathname, ".htm") != 0) || (strstr(req->pathname, ".cfg") != 0) )
      req->sht = 1;
    else
      req->sht = 0;
      
    //printf("req->pathname = %s sht = %d\n", req->pathname, req->sht);
#endif 	
	...
	send_r_request_ok(req);		/* All's well */
	{
		...
		if (bob > 0) {
			...
			memcpy(req->buffer + req->buffer_end,
					...
					bob);
#if defined(DNI)        
	    if (req->sht == 0)	/* if need sht then buffer_end seted by req_write() */
#endif						
			req->buffer_end += bob;
			req->filepos += bob;
	}
...
//KEVIN:07DEC05:BlankState+++
	SmtWzdState = atoi(nvram_get("SmtWzdDone"));
	if(SmtWzdState == 1) {
#ifdef CONFIG_WAN_LAN_MATCH_DETECTION
		WanLanFlag = atoi(nvram_get("WL_ip_match"));
		if(WanLanFlag == 0) {
			sprintf(pathname_with_index, "%s%s", req->pathname, directory_index);
		} else {
			//nvram_set("SmtWzdStep","1");
			sprintf(pathname_with_index, "%s%s", "/www/", "LIPChangeNotice.html");
		}
#else
		sprintf(pathname_with_index, "%s%s", req->pathname, directory_index);
#endif
		...
		//nvram_set("SmtWzdStep","1");
		sprintf(pathname_with_index, "%s%s", "/www/", "CA_Welcome.html");
	}
//KEVIN:BlankState---
	
#if defined(SUPPORT_ASP)
	//strcpy(req->pathname, directory_index/*pathname_with_index*/);        
#endif	
