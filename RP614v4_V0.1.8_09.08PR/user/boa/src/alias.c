...
/* for proxy http */
#define WWW_PATH1	"img/"
#define WWW_PATH2	"help/"
#define CGI_BIN_PATH	"cgi-bin/"
int
find_first(request *req, char *pathname)
{
	char *dummy, *home, *url, *path, *file;
	char buffer_temp[(2*MAX_HEADER_LENGTH) + 1];  /* by martin@tdp.cz */
	int deli = 0;
	
	//if (atoi(nvram_get("SmtWzdDone")) == 1)
	//	return 0;	
	strcpy(buffer_temp, pathname);
	url = buffer_temp;
	while (home = strsep(&url, "/")) {
		if (url) {
			deli++;		
			if ((deli == 3) || !strncasecmp(url, WWW_PATH1, sizeof(WWW_PATH1)-1) || !strncasecmp(url, WWW_PATH2, sizeof(WWW_PATH2)-1)
				|| !strncasecmp(url, CGI_BIN_PATH, sizeof(CGI_BIN_PATH)-1)) {
				if (!strncasecmp(url, CGI_BIN_PATH, sizeof(CGI_BIN_PATH)-1)) 
					sprintf(pathname, "/%s", url);						
				else	
					sprintf(pathname, "%s/%s", "/www", url);
				//printf("pathanme = %s\n", pathname);
				if (req->method == M_POST)				
					strcpy(req->request_uri, pathname);				
				return 1;
			}
		}		
		else 
...
int translate_uri(request * req)
{
  ...
  while (current) {
	...
	find_first(req, buffer);		/* for http proxy */
	req->pathname = strdup(buffer);
...
#ifdef SUPPORT_ASP
		if(req->method==M_POST)  return 1;
		if(strstr(req->request_uri,".")==NULL) return 1;
