...
//#endif	
  ...
  if (server_chroot)
  {
	...
	/* for upgrade firmware upgate response only */	
	//system("cp /www/erase_proc.html /var/tmp/erase_proc.html");
	
	//ch### >>
	if(POT_GetResolution()==0xffff)
	{
        POT_factoryDefault(1,30);
	}
	//ch### <<
	...
	while (1) {
		...
		//uint32 pot;
	        	...
	        	if((etime-stime)>=POT_INTERVAL*POT_GetResolution())
			{
				stime=etime;
				cnt++;
				POT_oneSecPass();
