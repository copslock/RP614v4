...
/* ATTACHED_DEVICES */
struct attached_devices
{
	int addrResv; //address reserved == 1
	unsigned char attached_host[34];
	unsigned char attached_mac[20];
	unsigned char attached_ip[20];
};
//struct attached_devices _attached_devices[254];
extern struct attached_devices _attached_devices[];
extern int atchedDvisCnt;

/* NON_ATTACHED_DEVICES */
struct non_attached_devices
{
	unsigned char attached_host[34];
	unsigned char attached_mac[20];
	unsigned char attached_ip[20];
...
//struct attached_devices _attached_devices[254];
extern struct non_attached_devices _non_attached_devices[];
extern int non_atchedDvisCnt;

struct dhcpc_rsvd_addr
{
	int addrBound;//the flag is to judge if any of mac or ip is found already reserved! If found, then addBound==1
	unsigned char rsvd_host[34];
	unsigned char rsvd_mac[20];
	unsigned char rsvd_ip[20];

};
//struct dhcpc_rsvd_addr _dhcpc_rsvd_addr[254];
extern struct dhcpc_rsvd_addr _dhcpc_rsvd_addr[];
extern int dhcpcRsvdAddrCnt;
...
extern client_req_t client_req;
