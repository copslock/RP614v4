...
static void perform_renew(void)
{
	...
	case BOUND:
		...
		/* <---added by chenyl : Start ---> */
		/* from RENEW_REQUESTED to RENEWING : it's only used for domain/wins re-get, so we translate state to
			RENEWING is appropriate */
		//state = RENEWING;
...
int udhcpc_main(int argc, char *argv[])
{
	...
	fprintf(f,"%d",getpid());	
	fclose(f);
	re865x_ioctl("eth0",RTL8651_IOCTL_SET_DHCPC_PID,getpid(),0,0);
#endif	
