...
int main(int argc, char *argv[])
{
	...
	if (strstr(base ? (base + 1) : argv[0], "init"))
		ret = init_main(argc, argv);
#if 0		
	else if (strstr(base ? (base + 1) : argv[0], "wanhook"))
		ret = udhcpc_wan(argc, argv);
	else if (strstr(base ? (base + 1) : argv[0], "ip-up"))
	{
		printf("simple_init::main: calling ip-up\n");
		ret = ipup_main(argc, argv);
	}
	else if (strstr(base ? (base + 1) : argv[0], "ip-down"))
	{
		printf("simple_init::main: calling ip-down\n");
		ret = ipdown_main(argc, argv);
	}
#endif	
	else if( strstr(base ? (base + 1) : argv[0], "StopWan") )
	{
		printf("argv[0] = %s argv[1]=%s argv[2]=%s\n", argv[0], argv[1], argv[2]); 
		if( argv[1] )
		{
			//if( strstr( argv[1], "eth0" ) )
			if (strstr( argv[1], "0"))
			{
				//stop_wan();
				start_wan();
	...
	//added by dvd.chen, 01/05/2006
	else if (strstr(base ? (base + 1) : argv[0], "hb_start"))
	{
		printf("simple_init::main: calling start_heartbeat()\n");
		start_heartbeat();
	}
	//end of dvd.chen
	else if (strstr(base ? (base + 1) : argv[0], "hb_connect"))
    		return hb_connect_main(argc, argv);
	else if (strstr(base ? (base + 1) : argv[0], "hb_disconnect"))
    		return hb_disconnect_main(argc, argv);
	else 
	{
		printf("simpleinit main::ERROR!");
	}
