...
static int _master_init(void)
{
    ...
    printf("initialising master DNS database\n");
...
#if defined(DNI)
{
#define NAME_1	"www.routerlogin.com"	
#define NAME_2	"routerlogin.com"	
#define NAME_3	"www.routerlogin.net"	
#define NAME_4	"routerlogin.net"	

    if (!getRomeCfgParamPtr(&pRomeCfgParam)) {
	struct in_addr	in;
	memcpy((char*)&in,(char*)pRomeCfgParam->ifCfgParam[1].ipAddr,sizeof(in));
    	add_nameip(NAME_1, sizeof(NAME_1), inet_ntoa(in));		    
    	add_nameip(NAME_2, sizeof(NAME_2), inet_ntoa(in));		    
    	add_nameip(NAME_3, sizeof(NAME_3), inet_ntoa(in));		    
    	add_nameip(NAME_4, sizeof(NAME_4), inet_ntoa(in));		    
	...
	*domain = 0;
    ...
    printf("%d records in master DNS database\n", dbc);
...
int master_lookup(unsigned char *msg, int len)
{
	...
	/* if smartwizard wizard not done, always response lan ipaddr */
	if (!getRomeCfgParamPtr(&pRomeCfgParam)) {	
#if defined(CONFIG_WAN_LAN_MATCH_DETECTION)
	  if ((pRomeCfgParam->BlankStateCfgParam.SmtWzdDone == 0)  ||
	  	(pRomeCfgParam->wanlan_ip_mat_det.LAN_WAN_FLAG == 1)){
#else
	  if (pRomeCfgParam->BlankStateCfgParam.SmtWzdDone == 0) {
#endif
	    x = begin_assembly(&query);
	    ...
	    compile_int(x, DNS_CLASS_INET);
	    //compile_long(x, DEFAULT_TTL);
	    //compile_long(x, 3);	    /* 3 secs */
	    compile_long(x, 1);	    /* 3 secs */	    
	    start_rdata(x);
	    //compile_long(x, rec->u.nameip.ipnum);
	    compile_long(x, *((unsigned long *)&(pRomeCfgParam->ifCfgParam[1].ipAddr)));	    
            //compile_long(x, 0xc0a80101);	    /* default 192.168.1.1 */
	    end_rdata(x);
