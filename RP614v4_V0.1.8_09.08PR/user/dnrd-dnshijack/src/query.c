...
static unsigned long total_quering=0;
#endif
...
query_t *query_add(domnode_t *dom, srvnode_t *srv, 
		   const struct sockaddr_in* client, char* msg, 
		   unsigned len, PACKET_INFO *lan_packet) {
#endif
  ...
  */
  ...
  #ifdef DNS_HIJACK
  if(lan_packet != NULL) //prevent udp_send_dummy fail
  {
  //if daddr is not 192.168.1.1 then must be user defined DNS server
    {
     unsigned long lan_ip;
     if (!getRomeCfgParamPtr(&pRomeCfgParam)) {
        
	 memcpy((char *)&lan_ip, (char *)pRomeCfgParam->ifCfgParam[1].ipAddr, sizeof(lan_ip));
		 
	 if( lan_packet->ip_hdr.daddr != lan_ip)
	 {	 	
		//memcpy(&(srv->addr.sin_addr), &(lan_packet->ip_hdr.daddr), 4);		
		memcpy(&(srv->realaddr.sin_addr), &(lan_packet->ip_hdr.daddr), 4);		
	 }
	 else 	/* lan ip is dns server */
		memcpy(&(srv->realaddr.sin_addr), &(srv->addr.sin_addr), 4);		
  ...
  memcpy( &(q->ether_hdr.h_source), &(lan_packet->ether_hdr.h_source), ETH_ALEN);
  memcpy( &(q->ether_hdr.h_dest), &(lan_packet->ether_hdr.h_dest), ETH_ALEN);
    
  memcpy( &(q->ip_hdr.saddr), &(lan_packet->ip_hdr.saddr), 4);
  memcpy( &(q->ip_hdr.daddr), &(lan_packet->ip_hdr.daddr), 4);     
     
  memcpy( &(q->udp_hdr.source_port), &(lan_packet->udp_hdr.source_port), 2);
  memcpy( &(q->udp_hdr.dest_port), &(lan_packet->udp_hdr.dest_port), 2);  
  }
  else
  {
  memset( &(q->ether_hdr.h_source), 0, ETH_ALEN);
  memset( &(q->ether_hdr.h_dest), 0, ETH_ALEN);
    
  memset( &(q->ip_hdr.saddr), 0, 4);
  memset( &(q->ip_hdr.daddr), 0, 4);     
     
  memset( &(q->udp_hdr.source_port), 0, 2);
  memset( &(q->udp_hdr.dest_port), 0, 2);   
  }
  #endif    
