...

{
	...
	*len = replylen;
	...
	//printf("Replying to query with cached answer.");
	*len = replylen;
	return 0;
    }
	...
	printf("Replying to query with \"Server failure\"");	
	//if (!set_srvfail(msg, *len)) return -1;
	//return 0;

    	*dptr = d;	
	return 1;	/* continue to forward */
	...
	//printf( "Forwarding the query to DNS server %s\n",
	//	  inet_ntoa(d->current->addr.sin_addr));    
    } else {
      log_debug(3, "All servers deactivated. Replying with \"Server failure\"");
      printf("All servers deactivated. Replying with \"Server failure\"");
      //if (!set_srvfail(msg, *len)) return -1;
      //return 0;
      
      *dptr = d;	/* continue to forward */
      return 1;
    }
...
#ifdef DNS_HIJACK
  FD_SET(raw_lan_sd,  &fdmaster);
  if (isock >= 0)  {
  	FD_SET(isock,  &fdmaster);
  }	
#ifdef ENABLE_TCP
  FD_SET(tcpsock, &fdmaster);
  maxsock = (tcpsock > raw_lan_sd) ? tcpsock : raw_lan_sd;
#else
  maxsock = (raw_lan_sd > isock) ? raw_lan_sd : isock;
#endif
  
#else //else DNS_HIJACK
  FD_SET(isock,   &fdmaster);
...
/*
 *  Lan side raw ip socket tear down as TCP and UDP
 */
	...
	tcp_handle_request();
      ...
      /*
       *  check isock, recv and do nothing
       */
      {
        char         	temp_msg[512+4]; // padd 00 01 00 01 4bytes
	int          	temp_maxsize = 512;
	int	     	temp_len;
	unsigned	temp_addr_len;
	struct sockaddr_in 	from_addr;		
	temp_addr_len = sizeof(struct sockaddr_in);

      	if ((isock >= 0) && FD_ISSET(isock, &fdread)) {
            temp_len = recvfrom(isock, temp_msg, temp_maxsize, 0,
		   (struct sockaddr *)&from_addr, &temp_addr_len);
	        if (temp_len < 0) {
