...
#ifdef DNS_HIJACK
/*Set interface ro promicious mode*/
int open_fd(char *intf)
{
    int f, s;
    struct ifreq ifr;
  /*create AF_INET socket to receive raw packets 0x3 means all packets,0800 is ethernet type*/
   if ((f = socket(AF_INET, SOCK_PACKET,ETH_P_ALL)) < 0) {
    //if ((f = socket(AF_INET, SOCK_PACKET, htons(0x3))) < 0) {
       	printf("f: Couldn't open socket");
	cleanexit(-1);
    }
     ...
     */
    //if ((f = socket(AF_INET, SOCK_RAW, IPPROTO_UDP)) < 0) {
	//log_msg(LOG_ERR, "isock: Couldn't open socket");
	//printf("f: Couldn't open socket");
	//cleanexit(-1);
    //}
    
    /*copy interface's name*/
    strcpy(ifr.ifr_name, intf);
    /*use ioctl combined SIOCGIFFLAGS to control hardware device eth1, eth0, socket_fd f*/
    ...
    /* set ifr with SIOCSIFFLAGS*/
    if ((s = ioctl(f, SIOCSIFFLAGS, &ifr)) < 0) {
        return(-1);
    }
    //log_msg(LOG_INFO,"Setting interface ::: %s ::: to promisc\n\n", intf); 
...
int getdevindex(int sock, char *name)
{
	...
	if(ioctl(sock, SIOCGIFINDEX, &ifr) == -1) 
		printf("I can't get device flags...");
...
{
    char            *buf=NULL, tmp_chr=0;
    unsigned long   sum = 0;
    unsigned short  *uph;
    int            i, cnt;

    /* UDP header checksum */
    uph =(unsigned short *)hdt_ptr;
    for (i=0; i<UDP_LENGTH/2; i++) {
        sum += uph[i];
    }
    len -= UDP_LENGTH;
