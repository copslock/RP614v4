...
srvnode_t *set_current(domnode_t *d, srvnode_t *s) {
  ...
  if (s) {
    if (d->roundrobin) {
      ...
      printf("Setting server %s for domain %s",
		inet_ntoa(s->addr.sin_addr), cname2asc(d->domain));
    } else {
  ...
  else 
  {
    log_msg(LOG_WARNING, "No active servers for domain %s", 
	    cname2asc(d->domain));
    printf("No active servers for domain %s", 
	    cname2asc(d->domain));	    
  }
  return (d->current = s);
}
...
srvnode_t *deactivate_current(domnode_t *d) {
  ...
  if (d->current) {
#ifdef DNS_HIJACK  	
    if ((d->current->realaddr.sin_addr.s_addr) != (d->current->addr.sin_addr.s_addr)) {
	d->current->send_time = 0;       	
     	return NULL;
    }	
    if (active_srvnodes(d->srvlist) > 1) {
#endif        
    	d->current->inactive = time(NULL);
    	log_msg(LOG_NOTICE, "Deactivating DNS server %s",
		      inet_ntoa(d->current->addr.sin_addr));
    	printf("Deactivating DNS server %s",
		      inet_ntoa(d->current->addr.sin_addr));   
#ifdef DNS_HIJACK 		      
    }
    else {
...
#ifdef DNS_HIJACK
      if ((s->realaddr.sin_addr.s_addr) != (s->addr.sin_addr.s_addr)) {
		memcpy(&(s->realaddr.sin_addr), &(s->addr.sin_addr), 4);		
      		//continue;
      }		
#endif        		    	
      s->inactive=now;
