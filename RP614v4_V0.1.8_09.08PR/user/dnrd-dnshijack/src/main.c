...
int main(int argc, char *argv[])
{
  /*    int                i;*/
...
#else //defined DN_HIJACK

    if ((raw_lan_sd = open_fd("eth1")) < 0) { //sniff lan port packets
        printf( "Error: can't get promicuous socket.\n");
        cleanexit(-1);
    }
    ...
    if ((isock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
	//log_msg(LOG_ERR, "isock: Couldn't open socket");
	printf("isock: Couldn't open socket");
	cleanexit(-1);
   }
    else {
	int opt = 1;
    ...
    if (bind(isock, (struct sockaddr *)&recv_addr, sizeof(recv_addr)) < 0) {
	//log_msg(LOG_ERR, "raw_lan_sd: Couldn't bind local address");
	printf("raw_lan_sd: Couldn't bind local address\n");
	cleanexit(-1);
    }
#endif    
...
#ifndef EMBED //embed is defined outside
	...
	*/
	...
	s->realaddr.sin_family = AF_INET;
	s->realaddr.sin_port   = htons(53);	
