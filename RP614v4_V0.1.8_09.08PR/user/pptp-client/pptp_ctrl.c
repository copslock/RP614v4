 ...
 *
 * $Id: pptp_ctrl.c,v 1.2 2004/10/28 11:09:31 chenyl Exp $
 */
...
#if 1
{
	...
	strncpy(packet.phone_num, pRomeCfgParam->pptpCfgParam.pptpid, sizeof(pRomeCfgParam->pptpCfgParam.pptpid));
	shmdt(pRomeCfgParam);
}
#endif    
...
void pptp_dispatch_ctrl_packet(PPTP_CONN * conn, void * buffer, size_t size) {
  ...
  case PPTP_CALL_CLEAR_NTFY:
	...
	fprintf(stderr, "Call closed (NTFY) (call id %d)", (int) call->call_id);
...
#ifdef RTL865X_PPTP	/* call_clear_notify from peer */
	  else
      	{
      		unsigned int callId;
		int idx;
		...
		idx = 0;
		while ((myCall = (PPTP_CALL *)vector_getPptpcall(conn->call, &idx)) != NULL)
		{
			if (myCall->peer_call_id == ntoh16(packet->call_id))
			{
				callId = myCall->call_id;
	      			if (vector_contains(conn->call, callId))
	      			{
					PPTP_CALL * call;
					vector_search(conn->call, callId, &call);
					pptp_call_destroy(conn, call);
	      			}
			}
		}		
