...
/* $Id: mtd.h,v 1.4 2005/12/02 01:10:51 alva_zhang Exp $ */
...
{
  struct file   *fd;
  unsigned int base; //mtd partition base address
  unsigned int size; //mtd partition size ;
 // unsigned int flag;
};

typedef struct mtd_flashdev
{
  unsigned int     buswidth;
  unsigned short partcnt;
  unsigned short erasize;
  
  unsigned int    chipbase;
  unsigned long chipsize; 
  
  unsigned int bdinfobase;
  unsigned int bdinfosize;
  
  unsigned int ccfgbase;
  unsigned int ccfgsize;
  
  unsigned int kernbase;
  unsigned int kernsize;
  
  unsigned int rootfsbase;
  unsigned int rootfssize;
  
  struct partinfo p[6];

}MTD_FlashDev;

extern struct mtd_flashdev mtd_dev;
...
#define MEMINFOINIT               _IOWR('M', 11, struct mtd_flashdev)
