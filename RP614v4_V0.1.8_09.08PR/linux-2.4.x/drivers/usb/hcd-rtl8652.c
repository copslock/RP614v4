...
static dma_addr_t rtl_map_single(void *ptr, size_t size, int direction)
{
 ...
 */
static void rtl_unmap_single(dma_addr_t dma_addr,
...
error:
	...
	usb_hcd_giveback_urb_rtl8652 (hcd, urb, 0);
	return 0;
}
 ...
 */
int usb_hcd_probe_rtl8652 (struct hc_driver *driver)
{
	void			*base;
	struct usb_bus		*bus;
	struct usb_hcd		*hcd;
	int			retval; 	
//	char			buf [8];
	int 			rtl8652_usb_irq=7;

	base=(void *)0xb8021000; //EHCI Capability and Operational  Register
...
clean_2:
	...
	if (request_irq (rtl8652_usb_irq, hcd_irq, SA_SHIRQ, hcd->description, hcd)
			!= 0) {
		err ("request interrupt irq:%d failed", rtl8652_usb_irq);
		retval = -EBUSY;
clean_3:
		driver->hcd_free (hcd);
		goto clean_2;
	}
	hcd->irq = rtl8652_usb_irq;

	hcd->regs = base;
	
//	hcd->region = region; //unknow ...
	
	printk (KERN_INFO "%s : irq %d,  %p\n",
		hcd->description, rtl8652_usb_irq, base);
...
//	hcd->product_desc = dev->name;
	...
	{
		printk("usb driver->start failed!\n");
...
EXPORT_SYMBOL (usb_hcd_probe_rtl8652);


#if 0  // on-chip usb 2.0 do not need those functions
 ...
 */
void usb_hcd_giveback_urb_rtl8652 (struct usb_hcd *hcd, struct urb *urb, struct pt_regs *regs)
{
...
#if 1
		rtl_unmap_single ( urb->transfer_dma,
				urb->transfer_buffer_length,
				usb_pipein (urb->pipe)
				    ? PCI_DMA_FROMDEVICE
				    : PCI_DMA_TODEVICE);
...
EXPORT_SYMBOL (usb_hcd_giveback_urb_rtl8652);
