...
static int __init ohci_hcd_init (void) 
{
	...
	unsigned int value;
	// this is for a fixed port when using an ALi USB host card	
	unsigned int ali_pci = 0x200, op_offset = 0x10, ali_port = 0x1c;
	int i;


	printk("start to emit TEST_PACKET\n");
	// initialize PCI interface and memory mapping address
	*(volatile unsigned int *)(0xbbd10104+ali_pci) = 0x2b00147;
	*(volatile unsigned int *)(0xbbd1010c+ali_pci) = 0x4000;
	*(volatile unsigned int *)(0xbbd10140+ali_pci) = 0x2010;

	// enable the USB ports

	// USBINTR - bit:2 Port Change Interrupt Enable
	*(volatile unsigned int *)(0xbbf03018+op_offset) = 0x4;

	// USBCMD - bit:0 Run
	*(volatile unsigned int *)(0xbbf03010+op_offset) = 0x1;

	// Port 3: Port Stauts and Control Register (Port Power, enable)
	*(volatile unsigned int *)(0xbbf03054+ali_port) = 0x1000; 

	// CONFIGFLAG :  Port routing control logic default-routes all ports to this host controller
	*(volatile unsigned int *)(0xbbf03050+op_offset) = 0x1;

	// USBSTS: bit:2 Port Change Status , write one to clear
	*(volatile unsigned int *)(0xbbf03014+op_offset) = 0x4;
	printk("0xbbf03054=%x\n",*(volatile unsigned int *)(0xbbf03054+ali_port));

	// Port 3: Port Stauts and Control Register (Coonect Stauts Change write one to clear)
	*(volatile unsigned int *)(0xbbf03054+ali_port) = 0x1802;

	// Port 3: Port Stauts and Control Register (Port Reset / make sure "PortEnabled" to zero)
	// must keep Port Reset bit keep in 1 for 2ms.
	
	*(volatile unsigned int *)(0xbbf03054+ali_port) = 0x1901;  
	
	printk("Port Status and Control Register =%x\n",
		*(volatile unsigned int *)(0xbbf03054+ali_port));

	for(i=0;i<100000;) { i++; }
	
	//  Port 3: Port Stauts and Control Register ( clear Port reset bit)
	*(volatile unsigned int *)(0xbbf03054+ali_port) = 0x1001;
	
	printk("final command=%x, port status=%x\n",
		*(volatile unsigned int *)(0xbbf03010+op_offset),
		*(volatile unsigned int *)(0xbbf03054+ali_port));

	
	// display all the USB registers
	for(i=0;i<0x78;i+=4) {
		printk("%x = %x\n", (0xbbf03000+i+op_offset),
			*(volatile unsigned int *)(0xbbf03000+i+op_offset));
	}

	
	printk("eye pattern/level/test.............................\n");

	// eye pattern/level/test
	// start TEST packet procedure
	...
	for(i=0;i<100000;) { i++; }	
	// USBCMD
	value = *(volatile unsigned int *)(0xbbf03010+op_offset);
	value &= 0xffffffcf;	
	// disable period & async schedule
	*(volatile unsigned int *)(0xbbf03010+op_offset)=value; // disable period & async
	

	printk("disable period & async schedule, Port Status and Control Register =%x\n",
		*(volatile unsigned int *)(0xbbf03054+ali_port));

	for(i=0;i<100000;) { i++; }
	// disable RUN -> set to STOP 
	value = *(volatile unsigned int *)(0xbbf03010+op_offset);
	value &= 0xfffffffe;
	*(volatile unsigned int *)(0xbbf03010+op_offset)=value; // disable Run bit

	for(i=0;i<100000;) { i++; }
	printk("disable RUN -> set to STOP ,Port Status and Control Register =%x\n",
		*(volatile unsigned int *)(0xbbf03054+ali_port));
	
	// check Halted bit (USBSTS: HCHalted ... wait for hardware halted )
	for(;;) {
		value = *(volatile unsigned int *)(0xbbf03014+op_offset);
		if(value & 0x1000)
			break;
	}
	// notes:the "set to STOP" and "set to suspend" sequence here
	// is reversed to that in EHCI specification (4.14) only for Synopsys
	// host controller

	// set to suspend state
	value = *(volatile unsigned int *)(0xbbf03054+ali_port);
	value |= 0x80;	
	*(volatile unsigned int *)(0xbbf03054+ali_port)=value;
	...
	*(volatile unsigned int *)(0xbbf03054+ali_port)=value;
	...
	// PORTSC bit 16~19 ( Port Test Control )
	// set test_packet mode	
	value |= 0x40000;	// TEST_PACKET
//	value |= 0x10000; 	// TEST_J_STATE
//	value |= 0x20000;	// TEST_K_STATE
//	value |= 0x30000;	// TEST_SE0_NAK
//	value |= 0x50000;	// TEST_FORCE_ENABLE
	*(volatile unsigned int *)(0xbbf03054+ali_port)=value;
	...
	printk("port is disconnected, port status=%x\n");
	for(;;);
#else
