			...
			switch(m_data[15]) {
				case 0:/* Station to switch Hello */
					...
					if(uint16Ptr[0] == 0xFFFF && uint16Ptr[1] == 0xFFFF && uint16Ptr[2] == 0xFFFF) {
						...
						if(_rtl8651_fwdEngineSend(FWDENG_FROMDRV, (void*)pPkt, local_vlanp->vid,-1)!=SUCCESS) 
						...
						if (dup_mBuf) {
							...
							if(_rtl8651_fwdEngineSend(FWDENG_FROMDRV, (void*)dup_mBuf->m_pkthdr, local_vlanp->vid,-1)!=SUCCESS) 
