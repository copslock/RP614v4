	...
	*************************************************************************************************/
	{
		...
		/**** Add by Timo***/
		char  action_drop[]="Drop";
		char  string_block[]="BLOCK";
		char  action_forward[]="Forward";
		//char  string_allow[]="ALLOW";
		/******END*****/
		...
		to_tm(tv.tv_sec, &tm);
		snprintf(msg_time, sizeof(msg_time), RTL8651_LOG_TIME_STRING, wday[tm.tm_wday], tm.tm_mday, wmonth[tm.tm_mon], tm.tm_year, tm.tm_hour, tm.tm_min, tm.tm_sec);
		//sprintf(msg_time, "%s", msg_init); //Timo Wu
		/**************************** Get message descriptor *****************************/
		snprintf(msg_desc, sizeof(msg_desc), "%s", log_desc[logNo - 1]);
		//sprintf(msg_desc, "%s", msg_init); //Timo Wu
		...
		if (info)
		{
			...
			switch (info->infoType)
			{
				case RTL8651_LOG_INFO_PKT:
					...
					{
						case IPPROTO_TCP:
							if(memcmp(action_drop, log_action[info->action],4)!=0) //Add by Timo Wu
								snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_PKT_TCPSTRING, RTL8651_LOG_INFO_PKT_TCPVARLIST);
							else
								snprintf(msg_detail, sizeof(msg_detail), "%s", DNI_LOG_INFO_PKT_DENIED);
							break;
						case IPPROTO_UDP:
							if(memcmp(action_drop, log_action[info->action],4)!=0)
				...
				case RTL8651_LOG_INFO_URL:
					...
					{
						case IPPROTO_TCP: //Add by Timo Wu
							if(memcmp(action_forward, log_action[info->action],4)==0) //Add by Timo Wu
								//snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_TCPSTRING, string_allow, RTL8651_LOG_INFO_URL_TCPVARLIST);
								goto ret;
							else if(memcmp(action_drop, log_action[info->action],4)==0)
								snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_TCPSTRING, string_block, RTL8651_LOG_INFO_URL_TCPVARLIST);
								
							break;
						case IPPROTO_UDP:
							if(memcmp(action_forward, log_action[info->action],4)==0) //Add by Timo Wu
								//snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_UDPSTRING, string_allow, RTL8651_LOG_INFO_URL_UDPVARLIST);
								goto ret;
							else if(memcmp(action_drop, log_action[info->action],4)==0)
								snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_UDPSTRING, string_block, RTL8651_LOG_INFO_URL_UDPVARLIST);
							break;
						default:
							if(memcmp(action_forward, log_action[info->action],4)==0) //Add by Timo Wu
								//snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_OTHERSTRING, string_allow, RTL8651_LOG_INFO_URL_OTHERVARLIST);
								goto ret;
							else if(memcmp(action_drop, log_action[info->action],4)==0)
								snprintf(msg_detail, sizeof(msg_detail), RTL8651_LOG_INFO_URL_OTHERSTRING, string_block, RTL8651_LOG_INFO_URL_OTHERVARLIST);
							break;
					}
					break;
				default:
		...
		/************** Generate Log message **************/
		{
			...
			#endif
			snprintf(logMsgLayout, sizeof(logMsgLayout), "%s\n", usrDefinedMsgfont); //Timo
...
//	printk("sendmail......pid=%d........%d\n",rtl8651_user_pid,60+idx);
