 ...
 * All rights reserved.                                                    
 * 
 *  $Header: /home/cvsroot/uClinux-dist/linux-2.4.x/drivers/net/re865x/re_core.c,v 1.287 2005/12/12 03:19:43 tony Exp $
 *
 * $Author: tony $
 *
 * Abstract:
...
// patch for: GPIO in - reset/factory default
#if 1
static uint32 gpio_push_down_cnt=0;
#define FACTORY_DEFAULT_TIME	50
extern void rtl865x_setGpioB0(int value);
#endif
...
#if 1
{
	if((REG32(PABCDAT)&0x80000000)==0) // push down reset button
	{
		if(gpio_push_down_cnt==0)
			printk("key down.... GPIO A7\n");
		
		gpio_push_down_cnt++;
		if (gpio_push_down_cnt >= FACTORY_DEFAULT_TIME) {
			if (gpio_push_down_cnt % 2)
				rtl865x_setGpioB0(1);
			else
				rtl865x_setGpioB0(0);
		}		
	...
	else if(gpio_push_down_cnt!=0)
	{
		printk("key up.... GPIO A7 timer:%u\n",gpio_push_down_cnt);
		if(gpio_push_down_cnt<FACTORY_DEFAULT_TIME)  // less 2.5secs , reset
		{
			printk("reset!\n");
			REG32(GIMR) = 0;
			REG32(WDTCNR)=0x0; 
		...
		{
			printk("factory default!\n");		
			sys_kill(rtl8651_user_pid,50); 
		}
		gpio_push_down_cnt=0;
	}
}
#endif	
}
...
{
	REG32(PABCCNR)&=0xfffeffff;
	REG32(PABDIR)|=0x00010000;
	REG32(PABCDAT)&=0xfffeffff;
	REG32(PABCDAT)|=(value<<16);
}

void rtl865x_setGpioB1(int value)
{
	REG32(PABCCNR)&=0xfffdffff;
	REG32(PABDIR)|=0x00020000;
	REG32(PABCDAT)&=0xfffdffff;
	REG32(PABCDAT)|=(value<<17);	
}
...
//		printk("move data to ip header 12\n");
	}else
#endif
	{
...
static fast_tx_fn airgo_tx = NULL;
fast_free_fn airgo_free = NULL;

void rtlairgo_fast_tx_register(void * fn, void * free_fn)
{
	airgo_tx = (fast_tx_fn)fn;
	airgo_free = (fast_free_fn)free_fn;
}
...
__IRAM_AIRGO int rtlairgo_fast_tx(struct rtl_pktHdr* pPkt)
{
	...
	if ((pPkt->ph_portlist==6) && ( pPkt->ph_extPortList < 8 ) && airgo_tx)
	{
		//printk("send packet to fast path %08X %08X\n",airgo_tx,pPkt);
		/*
...
#endif
static int32 rtl8651_fwdEnginecheck(void * pkthdr);
...
__IRAM_FWD static int32  rtl865x_fastRx(struct rtl_pktHdr* pPkt) {
	...
	/* if dhcp from eth0 then always to protocol stack 
	   rtl8651_fwdEngineInput will drop packets with ip conflict LAN/WAN */
	if (rtl8651_fwdEnginecheck((void *)pPkt) != SUCCESS) {
		retval=rtl8651_fwdEngineInput((void *)pPkt);
	}
	else
		retval = !SUCCESS;	/* to rtl865x_slowRx -> protocol stack */
...
uint32 vlan_lan_ip=0; //Add by Timo
uint32 vlan_lan_mask=0;
/* DNI begin */
...
static int32 rtl8651_fwdEnginecheck(void * pkthdr)
{
	...
	udp = (struct udphdr *)((uint8 *)ip + sizeof(struct ip));        
	//printk("ethType=%x, ip->ip_p=%d, udp->uh_sport=%d, udp->uh_dport=%d\n", 
	//	ethType, ip->ip_p, udp->uh_sport, udp->uh_dport);
	/* if dhcp from eth0 */	
        if ((ethType == 0x0800) && (ip->ip_p == IPPROTO_UDP) && (udp->uh_sport == 67) && (udp->uh_dport == 68) 
             && ((ip->ip_src.s_addr & vlan_lan_mask) == (vlan_lan_ip & vlan_lan_mask )) ) 
        	ret = SUCCESS;
        else
        	ret = !SUCCESS;
out:        	
...
#endif
				{
					assert(ppkthdr->ph_vlanIdx<RE865X_DEFAULT_ETH_INTERFACES);
...
#endif
			{
				assert(pPkt->ph_vlanIdx<RE865X_DEFAULT_ETH_INTERFACES);
...
static int re865x_open (struct net_device *dev)
{
	...
	if(!_re865xNIC_init){
		...
		if(SUCCESS!=swNic_init(128,128,128,128,128,128,128,16,16,PKT_BUF_SZ, rtl865x_fastRx ,NULL)){
			printk("Failed to initialize NIC!!!\n\n\n");
			return -1;
		}
#else
		if(SUCCESS!=swNic_init(64,64,16,PKT_BUF_SZ, rtl865x_fastRx ,NULL)){
			printk("Failed to initialize NIC!!!\n\n\n");
			return -1;
		}
#endif
...
__IRAM_TX static int re865x_start_xmit(struct sk_buff *skb, struct net_device *dev){
	...
	uint32 sendsize=0;
	uint32 last_seq=0; //for tcp	LSO
	int32 loop=0,retval,fastPath=FALSE,mss=1514-header_len;
	struct ip *iphdr=NULL;
	uint8 oldTcpFlag=0;
	uint8 toWireless=FALSE;
	uint8 isPPPoETagged = FALSE;
...
//	printk("skb->data %d\n",*(unsigned char*)skb->data);
	...
	if(iphdrOffset>=0)
	{
		...
		if(mBuf_attachHeaderLSO(realLen,dataLen,skb->data,skb,iphdr->ip_p,Mblk_list)==FAILED) goto no_buffer;
	}
	else
	{
		dataLen=realLen;
		header_len=0;	
		if(mBuf_attachHeaderLSO(realLen,dataLen,skb->data,skb,0,Mblk_list)==FAILED) goto no_buffer;
	}
	

	//printk("start_xmit protocol=%x dataLen=%d\n",iphdr->ip_p,dataLen);
	/* init packet property before sending it in ROMEDRV */
	while(1)
	{
		Mblk=Mblk_list[loop];	
...
int  __init rtl865x_probe (void)
{
	...
	REG32(SYSTEM_BASE) |= ((1 << 24) | (1 << 25));    /* reset queue, full reset */	
	...
	/* for netgear spec */
	{
  		rtl8651_tblDrvInitPara_t tblDrvPara;
  		
  		bzero(&tblDrvPara,sizeof(rtl8651_tblDrvInitPara_t));
  		tblDrvPara.dialSessionNumber=1;
  		//tblDrvPara.urlfilterDBSize = 255;
  		tblDrvPara.urlfilterDBSize = 50;  		
  		tblDrvPara.urlfilterStringLen = 49;
  		tblDrvPara.pathfilterStringLen = 49;
  		//tblDrvPara.urlPktfilterDBSize = 255;
 		tblDrvPara.urlPktfilterDBSize = 50;  		
  		//tblDrvPara.routingTableSize = 16;
  		tblDrvPara.routingTableSize = 20;  		
  		tblDrvPara.naptServerPortEntryNumber = 40;
		tblDrvPara.policyRules = 80;    		
  		INIT_CHECK(rtl8651_tblDrvInit(&tblDrvPara));
 	}
	...
	*/
	...
	/* for netgear spec */
	{
  		rtl8651_fwdEngineInitPara_t fwdEnginePara;
  		
  		memset(&fwdEnginePara, 0xff, sizeof(rtl8651_fwdEngineInitPara_t));
  		fwdEnginePara.MaxFragSubPktCnt=50;
  		INIT_CHECK(rtl8651_fwdEngineInit(&fwdEnginePara));
 	}		
	...
	REG32(PABDIR) &=  0x7fffffff; /* set port a-7 to input */	
...
#else
	INIT_CHECK(mBuf_init(128,  0/* use external cluster pool */, 128, PKT_BUF_SZ, 0)); 
#endif
...
//		dev->features |= (NETIF_F_IP_CSUM|NETIF_F_NO_CSUM|NETIF_F_HW_CSUM|NETIF_F_SG);
		...
		if(i==0){//only need to do once. 
			...
			if(rc)
				...
				printk("*** IRQ %d allocated for 8650 PCM\n", ICU_PCM);
#endif
...
static int re86xx_set_mac(struct net_device *dev,int id,int mac_addr)
{
	int32 retval;
	char *mac=(char*)mac_addr;
	retval = rtl8651_assignVlanMacAddress(id, (ether_addr_t*)mac, 1);	
	if (retval)
	{
		printk("rtl8651_assignVlanMacAddress: ret %d\n", retval);
		return retval;
	}
	printk("set \"%s\" mac to --> %02x:%02x:%02x:%02x:%02x:%02x\n",dev->name,mac[0]&0xff,mac[1]&0xff,mac[2]&0xff,mac[3]&0xff,mac[4]&0xff,mac[5]&0xff);
...
static int re86xx_add_vlan(struct net_device *dev,int id,int port,int mac_addr)
{
	...
	printk("add software vlan (2765) retval: %d\n", rc);
	retval = rtl8651_specifyVlanFilterDatabase(2765, fid);
	printk("rtl8651_specifyVlanFilterDatabase (2765) retval: %d\n", retval);
	for (i = 0; i < 5; i++) {
		retval = rtl8651_addVlanPortMember(2765, i);
		printk("rtl8651_addVlanPortMember (2765) port %d retval: %d\n", i, retval);
	}
	...
	if (retval)
	{
		printk("rtl8651_specifyVlanFilterDatabase: ret %d\n", retval);
	}
	...
	switch(args[0])
	{
		...
		case RTL8651_IOCTL_REPROBE:
			...
			*pRet = ret;
			break;
		case SET_MAC:
			re86xx_set_mac(dev,args[1],args[2]);
			break;
		case ADD_VLAN:
...
//uint32 		
		...
		case SET_PPPD_IDLE_TIME:
			...
			_rtl86xx_dev.pppoeIdleTimeout[args[1]]=0;
//			printk("set_pppd_idle_time=%ld in dsid=%ld\n",args[3],args[1]);
			break;
		case GET_PPPD_IDLE_TIME:
//			printk("get pppd idle =%d\n",_rtl86xx_dev.pppoeIdleTimeout[args[1]]);
			copy_to_user((char*)args[2],(char*)&_rtl86xx_dev.pppoeIdleTimeout[args[1]],4);
			if(_rtl86xx_dev.pppoeIdleTimeout[args[1]]==1) printk("PPPD idle timeout....\n");
			break;
...
*/		
		...
		case RTL8651_IOCTL_ADDTRIGGERPORTMAPPINGRULE:
			ptr=(uint32 *)args[1];
			pU32_1=(uint32 *)ptr[0];
			pU8 =	(uint8 *)ptr[1];
			pU16_1 =(uint16 *)ptr[2];
...
#if 0			
			{
				_rtl8651_triggerPort_param_t *mapping;				
				mapping=(_rtl8651_triggerPort_param_t *)pU32;			
				printk("port start:%d ~ end:%d age=%d localip=%x~%x\n",*pU16_1,*pU16_2
					,mapping->age
					,mapping->tp_triggeringInsideLocalIpAddrStart
					,mapping->tp_triggeringInsideLocalIpAddrEnd);
			}
#endif
			rc = rtl8651_addTriggerPortMappingRule((uint32)*pU32_1,(uint8)*pU8,(uint16)*pU16_1,(uint16)*pU16_2,(_rtl8651_triggerPort_param_t *)pU32);
			pU32=(int32 *)args[3];
			*pU32=rc;			
			break;
			
		case RTL8651_IOCTL_SETGPIOB0:
			//printk("set B0 to %d\n",(uint32)args[1]);
			rtl865x_setGpioB0(args[1]);
			break;

		case RTL8651_IOCTL_SETGPIOB1:
			//printk("set B1 to %d\n",(uint32)args[1]);
			rtl865x_setGpioB1(args[1]);
			break;
...
//#ifdef _RTL_URL_FILTER_TRUSTED_USER
		...
		{			
			...
			printk("rtl8651_addUrlFilterTrustedUser...ip=%x,ret=%d\n",(uint32)args[1],ret);
			pU32 = (uint32*)args[3];
			*pU32 = ret;
			break;
		}
		...
		{
			printk("rtl8651_flushUrlFilterTrustedUser...\n");
			rtl8651_flushUrlFilterTrustedUser();
			break;
		}
#endif
		case RTL8651_IOCTL_SETPORTPOWERDOWN:
		{ 
				
				pU32=(uint32 *)args[1];
				rtl8651_setEthernetPortPowerDown(pU32[0], pU32[1]);
				break;
		}
...
int32 rtl8651_setEthernetPortPowerDown(uint32 port, int32 powerdown)
{
        uint32 value;
        if(port >= 6)
                return -1;
        ...
        if(powerdown){
                ...
                (*((volatile uint32 *)((0x020 + (0xBC800000 + 0x00000000)))))=(value & (~(1<<11)));
...
int32 reCore_getDEVIdByVlanIdx(uint8 vlanIdx)
{
	if (vlanIdx < RTL8651_VLAN_NUMBER)
	{
		...
		dev = _rtl86xx_dev.dev[vlanIdx];

		if ((dev == NULL) || (dev->priv == NULL))
		{
			printk("%s %d: Device for vlan index [%d] is invalid\n", __FUNCTION__, __LINE__, vlanIdx);
			return -1;
		}
		...
		{
			printk("%s %d: Private parameters for vlan index [%d] is invalid\n", __FUNCTION__, __LINE__, vlanIdx);
			return -1;
		}
	...
	{
		printk("%s %d: Error: invalid vlan index: %d\n", __FUNCTION__, __LINE__, vlanIdx);
		return -1;
	}
}
