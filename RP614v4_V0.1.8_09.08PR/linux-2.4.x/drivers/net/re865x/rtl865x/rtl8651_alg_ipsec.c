# 1 "rtl8651_alg_ipsec.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_alg_ipsec.c"
# 126 "rtl8651_alg_ipsec.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 127 "rtl8651_alg_ipsec.c" 2
# 1 "assert.h" 1
# 128 "rtl8651_alg_ipsec.c" 2
# 1 "types.h" 1
# 129 "rtl8651_alg_ipsec.c" 2
# 1 "rtl_queue.h" 1
# 130 "rtl8651_alg_ipsec.c" 2
# 1 "rtl_errno.h" 1
# 131 "rtl8651_alg_ipsec.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
static struct isakmp_data_s *_rtl8651_isakmpDb=((void *)0);
# 174 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_findIsakmpIn(uint32 peer_ip,uint32 alias_ip,uint64 icookie,uint64 rcookie){
        int i;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;

        for(i = 0 ; i < algMaxSession.ipsecMaxSession ; i++){
                if(tp->peer_ip == peer_ip &&
                   tp->alias_ip == alias_ip &&
                   ((tp->icookie == icookie )||(tp->icookie==0)||(icookie==0)) &&
                   ((tp->rcookie == rcookie)||(tp->rcookie==0)||(rcookie==0)) &&
                   tp->state == 1){
                        tp->idle_timer = 0;
                        tp->rcookie = rcookie;
                        tp->icookie = icookie;
...
# 207 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_findIsakmpOut(uint32 local_ip,uint32 peer_ip,uint64 icookie,uint64 rcookie){
        int i;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;
        ...
        for(i = 0 ; i < algMaxSession.ipsecMaxSession ; i++){
                if(tp->peer_ip == peer_ip &&
                   ...
                   ((tp->icookie== icookie)||(icookie==0) ||(tp->icookie==0)) &&
                   ((tp->rcookie== rcookie)||(rcookie==0) ||(tp->rcookie==0)) &&
...
# 242 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_addIsakmp(uint32 local_ip,uint32 peer_ip,uint32 alias_ip,
                uint64 icookie,uint64 rcookie,uint32 dsid){
        int i;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;
...
# 276 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_findEspIn(uint32 peer_ip,uint32 alias_ip,uint32 ispi){
        int i;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;
...
# 307 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_findEspOut(uint32 local_ip,uint32 peer_ip, uint32 ospi){
        int i;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;
...
# 339 "rtl8651_alg_ipsec.c"
static struct isakmp_data_s*
_rtl8651_addEsp(uint32 local_ip,uint32 peer_ip,uint32 ospi,uint32 ispi){
        ...
        for(i = 0 ; i < algMaxSession.ipsecMaxSession ; i++){
                if(tp->state == 1 && tp->peer_ip == peer_ip
                   && tp->local_ip == local_ip){
                        tp->idle_timer = 0;
                    tp->ospi = ospi;
                    tp->ispi = ispi;
        ...
        {
                ipaddr_t extip;
                extip=_rtl8651_getExtIpAddrByDsid(0);
                if(extip)
                {
                        tp = _rtl8651_addIsakmp(local_ip,peer_ip,extip,0,0,0);
                        return tp;
                }
        }
        return ((void *)0);
}
# 382 "rtl8651_alg_ipsec.c"
static int32 _rtl8651_l4EspNaptAliasIn(struct rtl_pktHdr *phdr,struct ip * pip)
{
        uint32 i_spi;
        struct isakmp_data_s *link;
        if((alg_supports[phdr->ph_un_sw.ROMEDRV_USG._ph_dsid]&(0x00000002|0x40000000))==0) return 0;

        i_spi = ((uint32) (*((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))<<24)|(*(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+1)<<16)|(*(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+2)<<8) | *(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+3));

        link = _rtl8651_findEspIn(((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3)), ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3)), i_spi);

        if(link == ((void *)0)){
                if (alg_serverips[0][1] == 0)
                {
                        return 34;
                }

                link = _rtl8651_addEsp(alg_serverips[0][1], ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3)),0, i_spi);
        }

        if (link != ((void *)0))
        {
                uint8 *dst;
                dst=(uint8 *)&pip->ip_dst;
                dst[0]=(link->local_ip>>24)&0xff;
                dst[1]=(link->local_ip>>16)&0xff;
                dst[2]=(link->local_ip>>8)&0xff;
                dst[3]=link->local_ip&0xff;
                phdr->ph_dip = link->local_ip;
...
# 430 "rtl8651_alg_ipsec.c"
static int32 _rtl8651_l4EspNaptAliasOut(struct rtl_pktHdr *phdr,struct ip *pip)
{

        uint32 o_spi;
        struct isakmp_data_s *link;
        if((alg_supports[phdr->ph_un_sw.ROMEDRV_USG._ph_dsid]&(0x00000002|0x40000000))==0) return 0;
        o_spi = ((uint32) (*((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))<<24)|(*(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+1)<<16)|(*(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+2)<<8) | *(((uint8*)((uint32 *)((int8 *) pip + ((*(uint8*)pip&0xf) << 2))))+3));

        link = _rtl8651_findEspOut(((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3)), ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3)), o_spi);

        if(link == ((void *)0)){
                link = _rtl8651_addEsp(((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3)), ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3)), o_spi,0);
        }
        ...
        if (link != ((void *)0))
        {
                ...
                ptr=(uint8 *)&pip->ip_src;
                ptr[0]=(link->alias_ip>>24)&0xff;
                ptr[1]=(link->alias_ip>>16)&0xff;
                ptr[2]=(link->alias_ip>>8)&0xff;
                ptr[3]=(link->alias_ip)&0xff;
                phdr->ph_sip = link->alias_ip;
...
# 474 "rtl8651_alg_ipsec.c"
int32 rtl8651_l4AliasHandleIpsecClientInbound(struct rtl_pktHdr *pkt, struct ip *pip,
                                           struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s * link)
{
        struct isakmp_data_s *tb;
        struct udphdr *ud;
        uint64 *dptr, icookie, rcookie;
        char compstr[12]={0};

        if((alg_supports[pkt->ph_un_sw.ROMEDRV_USG._ph_dsid]&(0x00000002|0x40000000))==0) return 0;
        ...
        dptr = (uint64 *) (ud+1);
        rtl8651_memcpy(&icookie,dptr,8);
        dptr++;
        rtl8651_memcpy(&rcookie,dptr,8);

        if(memcmp((uint8*)dptr,compstr,12)!=0)
        {

                tb = _rtl8651_findIsakmpIn(link->dstIpAddr,link->insideGlobalIpAddr,icookie,rcookie);

                if(tb == ((void *)0)){

                        if((alg_supports[pkt->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x40000000)==0) return 131;



                        rtl8651_addUpnpMap(0x08, 0, 0,
                                        link->insideGlobalIpAddr, 4500,
                                        link->insideLocalIpAddr, 4500);


                        tb = _rtl8651_addIsakmp(alg_serverips[pkt->ph_un_sw.ROMEDRV_USG._ph_dsid][1],link->dstIpAddr,
                                                                        link->insideGlobalIpAddr,icookie,rcookie,pkt->ph_un_sw.ROMEDRV_USG._ph_dsid);
                        if(tb == ((void *)0)){
                                return 0;
                        }
               }

                link->insideLocalIpAddr = tb->local_ip;
        }

        return 0;
}
# 535 "rtl8651_alg_ipsec.c"
int32 rtl8651_l4AliasHandleIpsecClientOutbound(
                ...
                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
        ...
        if(memcmp((uint8*)dptr,compstr,12)!=0)
        {
                ...
                tb = _rtl8651_findIsakmpOut(link->insideLocalIpAddr,link->dstIpAddr,icookie,rcookie);
                if(tb == ((void *)0)){

                        if((alg_supports[pkt->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000002)==0) return 131;

                        tb = _rtl8651_addIsakmp(link->insideLocalIpAddr,link->dstIpAddr,
                                                                        link->insideGlobalIpAddr,icookie,rcookie,pkt->ph_un_sw.ROMEDRV_USG._ph_dsid);
                        if(tb == ((void *)0)){
                                return 0;
                        }
                ...
                if(link->insideLocalPort==500)
                {

                        if(link->insideGlobalPort!=500)
                        {

                        rtl8651_addUpnpMap(0x04, link->dstIpAddr, 500,
                                        link->insideGlobalIpAddr, 500,link->insideLocalIpAddr, 500);

                                link->insideGlobalPort = 500;
                        }
                }
        }
        return 0;
}
# 602 "rtl8651_alg_ipsec.c"
int32 rtl8651_l4AliasIpsecInit(void *arg){



        if(_rtl8651_isakmpDb) rtlglue_free(_rtl8651_isakmpDb);
        _rtl8651_isakmpDb = (struct isakmp_data_s *)rtlglue_malloc(algMaxSession.ipsecMaxSession*(sizeof(struct isakmp_data_s)));
        memset(_rtl8651_isakmpDb,0,algMaxSession.ipsecMaxSession*(sizeof(struct isakmp_data_s)));
        rtl8651_l4EspNaptAliasIn = _rtl8651_l4EspNaptAliasIn;
        rtl8651_l4EspNaptAliasOut = _rtl8651_l4EspNaptAliasOut;
        return 0;
}
...
void rtl8651_l4AliasIpsecReInit(void){
        ...
        if(_rtl8651_isakmpDb) memset(_rtl8651_isakmpDb,0,algMaxSession.ipsecMaxSession*(sizeof(struct isakmp_data_s)));
        rtl8651_l4EspNaptAliasIn = _rtl8651_l4EspNaptAliasIn;
        rtl8651_l4EspNaptAliasOut = _rtl8651_l4EspNaptAliasOut;
}
# 632 "rtl8651_alg_ipsec.c"
void
rtl8651_l4AliasIpsecHouseKeeping(void){
        int i,j;
        struct isakmp_data_s *tp;
        tp = _rtl8651_isakmpDb;
        ...
        for(i = 0 ; i < algMaxSession.ipsecMaxSession ; i++){
                ...
                if(tp == ((void *)0) || _rtl8651_isakmpDb == ((void *)0))
                                return;
                if(tp->state == 0)
                        continue;
                ...
                for(j=0;j<2;j++)
                        if((alg_supports[j]&0x00000002)==0)
                        {
                                if(tp->dsid==j) tp->state = 0;
                        }

                tp->idle_timer++;
                if(tp->idle_timer > 600 ){
                        tp->state = 0;
                }
                tp++;
        }
