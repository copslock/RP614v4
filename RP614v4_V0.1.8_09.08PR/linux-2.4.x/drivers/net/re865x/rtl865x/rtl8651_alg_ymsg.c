# 1 "rtl8651_alg_ymsg.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_alg_ymsg.c"
# 94 "rtl8651_alg_ymsg.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 95 "rtl8651_alg_ymsg.c" 2
# 1 "assert.h" 1
# 96 "rtl8651_alg_ymsg.c" 2
# 1 "types.h" 1
# 97 "rtl8651_alg_ymsg.c" 2
# 1 "rtl_queue.h" 1
# 98 "rtl8651_alg_ymsg.c" 2
# 1 "rtl_errno.h" 1
# 99 "rtl8651_alg_ymsg.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
# 106 "rtl8651_alg_ymsg.c" 2



static uint8 *_rtl8651_l4_ymsg_alias_buf[1];
static ipaddr_t triggerIP=0;

int32 rtl8651_l4AliasHandleYmsgClientOutbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        struct ip *spip;
        struct tcphdr *tcp,*stcp;
        struct ymsg_command *ymsg;
        int32 hlen, tlen, dlen, off_set, i ,size_delta;
        int8 *sptr, *newpacket;
        int8 sflag=0, http[8]="http://",*ymsg_content;
        int8 route_ip[22]={0};
        ipaddr_t src,dest;
        struct rtl_mBuf *m = pkthdrPtr->PKTHDRNXT.mbuf_first;


        if((alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000040)==0) return 0;

        src=(((uint32) (*((uint8*)(&pip->ip_src.s_addr))<<24)|(*(((uint8*)(&pip->ip_src.s_addr))+1)<<16)|(*(((uint8*)(&pip->ip_src.s_addr))+2)<<8) | *(((uint8*)(&pip->ip_src.s_addr))+3)));
        dest=(((uint32) (*((uint8*)(&pip->ip_dst.s_addr))<<24)|(*(((uint8*)(&pip->ip_dst.s_addr))+1)<<16)|(*(((uint8*)(&pip->ip_dst.s_addr))+2)<<8) | *(((uint8*)(&pip->ip_dst.s_addr))+3)));



        {
                triggerIP = src;
                rtl8651a_addTriggerPortRule(pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid,2,5000,5001,0,0,0,(triggerIP));
                rtl8651a_addTriggerPortRule(pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid,2,5055,5055,0,0,0,(triggerIP));
        }
# 146 "rtl8651_alg_ymsg.c"
        tcp = (struct tcphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));
        ymsg = (struct ymsg_command *)((uint8 *)tcp + ((tcp->th_off_x & 0xf0) >> 2));
        ymsg_content = (void *)ymsg + sizeof(struct ymsg_command);

        off_set = ((uint8 *)pip) - m->m_data;

        if(off_set < 0 || off_set > 40 || tb == ((void *)0)) return -1;

        if(memcmp(ymsg->ymsg,"YMSG",4)!=0) return 0;
        if((ymsg->service)!=77) return 0;





        if (m->m_pkthdr->ph_len > 576) return -1;


        newpacket = (int8 *)_rtl8651_l4_ymsg_alias_buf[0];
        ...
        spip = (struct ip*)newpacket;
        stcp = (struct tcphdr *) ((int8 *) spip + ((spip->ip_vhl &0xf) << 2));
        sptr = (int8 *) newpacket+hlen+sizeof(struct ymsg_command);



        for(i=0;i<(ymsg->length)-10;i++)
        {
                if((sflag!=0)&&(ymsg_content[i]=='/')) break;
                if((sflag==0)&&(memcmp((void *)(ymsg_content+i),http,7)==0)) { i+=7; sflag=i; }
        }

        if(sflag==0) return 0;

        sprintf(route_ip,"%u.%u.%u.%u:5002",(tb->insideGlobalIpAddr>>24)&0xff,
                (tb->insideGlobalIpAddr>>16)&0xff,
                (tb->insideGlobalIpAddr>>8)&0xff,
                (tb->insideGlobalIpAddr)&0xff);

        size_delta=strlen(route_ip)-(i-sflag);


        sptr+=sflag;
        rtl8651_memcpy(sptr,route_ip,strlen(route_ip));
        rtl8651_memcpy(sptr+strlen(route_ip),ymsg_content+i,(ymsg->length)-i+1);


        {
                int32 delta;
                tb->tcpAckModified = 1;
                delta = _rtl8651_l4GetDeltaSeqOut(spip, stcp, tb);
                _rtl8651_l4AddSeq(spip, tb, delta + size_delta);
        }

        spip->ip_len = ((spip->ip_len)+size_delta);


        stcp->th_sum = 0;
        spip->ip_sum = 0;
        stcp->th_sum = _rtl8651_tcpChecksum(spip);
        spip->ip_sum = _rtl8651_ipChecksum(spip);

        mBuf_copyToMbuf(m, off_set, (spip->ip_len), newpacket);

        do {} while (0);



                rtl8651_addUpnpMap(0x02|0x04, 0, 0,
                                tb->insideGlobalIpAddr, 5002,
                                tb->insideLocalIpAddr, 80);


        return 0;
}

static int32 init_ymsg = 0;
int32 rtl8651_l4AliasYmsgInit(void *arg){


        if(init_ymsg)
                return 0;

        _rtl8651_l4_ymsg_alias_buf[0] = (uint8 *)rtlglue_malloc(576 +64);
        do {} while (0);
...
void rtl8651_l4AliasYmsgReInit()
{
        triggerIP=0;
}
