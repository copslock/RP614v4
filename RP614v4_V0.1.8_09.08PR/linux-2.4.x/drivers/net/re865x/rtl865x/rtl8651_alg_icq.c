...
# 62 "rtl8651_alg_icq.c" 2
# 1 "assert.h" 1
# 63 "rtl8651_alg_icq.c" 2
# 1 "types.h" 1
# 64 "rtl8651_alg_icq.c" 2
# 1 "rtl_queue.h" 1
# 65 "rtl8651_alg_icq.c" 2
# 1 "rtl_errno.h" 1
# 66 "rtl8651_alg_icq.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
# 73 "rtl8651_alg_icq.c" 2
static int client_session=0;
static ipaddr_t localip=0;

int32 rtl8651_l4AliasHandleIcqClientOutbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        struct tcphdr *tcp;
        struct icq_command *icq;
        struct icq_command2 *icq2;
        int32 hlen, tlen, dlen, idx=0;
        uint8 *payload;
        char buf[1500];

        if((alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000020)==0) return 0;



        if(localip!=tb->insideLocalIpAddr)
        {
                localip=tb->insideLocalIpAddr;

                rtl8651_addUpnpMap(0x02|0x04, 0, 0,
                        tb->insideGlobalIpAddr, 5190,
                        tb->insideLocalIpAddr, 5190);
        }


        tcp = (struct tcphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));
        payload = ((uint8 *)tcp + ((tcp->th_off_x & 0xf0) >> 2));

        tlen = (pip->ip_len);
        hlen = ((pip->ip_vhl & 0xf) + (( tcp->th_off_x >>4)&0xf)) << 2;
        dlen = tlen - hlen;

        while(idx+sizeof(struct icq_command)<dlen)
        {
                rtl8651_memcpy(buf,payload+idx,dlen-idx);
                icq = (struct icq_command *)buf;

                if((icq->id==0x2a)&&(icq->channel==0x02)&&((icq->snac1)==0x01)&&((icq->snac2)==0x1e))
                {
                        if(((icq->tlv6)==0x00060004)&&((icq->tlv8)==0x00080002)&&((icq->tlv12)==0x000c))
                                {
                                        ...
                                        rtl8651_addUpnpMap(0x02|0x08, 0, 0,
                                                        tb->insideGlobalIpAddr, 24500+client_session,
                                                        tb->insideLocalIpAddr, (icq->port));




                                        {
                                                uint16 *icqport=(uint16*)(payload+idx+40);
                                                *icqport=(24500+client_session);
                                        }
                                        ++client_session;
                                        client_session%=5;
                                        return 0;
        ...
        if(sizeof(struct icq_command2)+4>dlen) return 0;

        rtl8651_memcpy(buf,payload,dlen);
        icq2 = (struct icq_command2 *)buf;

        if((icq2->id==0x2a)&&(icq2->channel==0x02)&&((icq2->snac1)==0x04)&&((icq2->snac2)==0x06))
        {
                uint16 port;
                port=(*(payload+dlen-4)&0xff)|((*(payload+dlen-3)&0xff)<<8);
                ...
                {
                        rtl8651_addUpnpMap(0x02|0x04, 0, 0,
                                tb->insideGlobalIpAddr, port,
                                tb->insideLocalIpAddr, port);
                }


                port=(*(payload+dlen-8)&0xff)|((*(payload+dlen-7)&0xff)<<8);
...
int32 rtl8651_l4AliasHandleIcqServerOutbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        struct tcphdr *tcp;
