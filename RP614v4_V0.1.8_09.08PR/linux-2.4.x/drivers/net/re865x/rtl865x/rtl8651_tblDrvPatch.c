# 1 "rtl8651_tblDrvPatch.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_tblDrvPatch.c"
# 14 "rtl8651_tblDrvPatch.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 16 "rtl8651_tblDrvPatch.c" 2
# 1 "rtl8651_tblDrvPatch.h" 1
# 51 "rtl8651_tblDrvPatch.h"
...
# 17 "rtl8651_tblDrvPatch.c" 2
# 34 "rtl8651_tblDrvPatch.c"
               int32 RTL8651_SETASICVLAN( rtl8651_tblDrv_vlanTable_t *vt )
{
        rtl865x_tblAsicDrv_vlanParam_t vlan;
        extern uint32 phyPortLinkStatus;

        vlan.broadcastToCpu = vt->broadcastToCpu;
        vlan.enableRoute = vt->softRoute? 0: vt->ipAttached;
        vlan.inAclEnd = vt->inAclEnd;
        vlan.inAclStart = vt->inAclStart;
        vlan.internal = vt->internal;
        vlan.macAddr = vt->macAddr;
        vlan.macAddrNumber = vt->macAddrNumber;
        vlan.memberPortMask = vt->memberPortMask&phyPortLinkStatus;
        vlan.mtu = vt->mtu;
        vlan.outAclEnd = vt->outAclEnd;
        vlan.outAclStart = vt->outAclStart;
        memcpy(vlan.portState, DrvTbl.spanningTree[DrvTbl.filterDB[(vt)->fid].sid].portState, sizeof(vlan.portState));
        vlan.promiscuous = vt->promiscuous;
        vlan.untagPortMask = vt->untagPortMask;
        vlan.valid = vt->valid;
        vlan.vid = vt->vid;
        vlan.DMZFlag = vt->DMZflag;
        return rtl8651_setAsicVlan( &vlan );
}
                                      ...
                                      uint32 * untagPortMask, uint32 * macNumber, uint32 * mtu )
{
        ...
        if ( mac ) *mac = vlan.macAddr;
        if ( mbr ) *mbr = vlan.memberPortMask;
        if ( inAclStart ) *inAclStart = vlan.inAclStart;
        if ( inAclEnd ) *inAclEnd = vlan.inAclEnd;
        if ( outAclStart ) *outAclStart = vlan.outAclStart;
        if ( outAclEnd ) *outAclEnd = vlan.outAclEnd;
        if ( internalIntf ) *internalIntf = vlan.internal;
        if ( enableRoute ) *enableRoute = vlan.enableRoute;
        if ( portState ) memcpy( portState, vlan.portState, sizeof(vlan.portState) );
        if ( broadcastToCpu ) *broadcastToCpu = vlan.broadcastToCpu;
        if ( promiscuous ) *promiscuous = vlan.promiscuous;
        if ( untagPortMask ) *untagPortMask = vlan.untagPortMask;
        if ( macNumber ) *macNumber = vlan.macAddrNumber;
        if ( mtu ) *mtu = vlan.mtu;
                                         ...
                                         uint32 * nextHopRow, uint32 * nextHopColumn, uint32 * pppoeIdx)
{
        ...
        if(index >= 8 || ipAddr == ((void *)0) || ipMask == ((void *)0) || process == ((void *)0) || vidx == ((void *)0) ||
                arpStart == ((void *)0) || arpEnd == ((void *)0) || nextHopRow == ((void *)0) || nextHopColumn == ((void *)0) || pppoeIdx == ((void *)0))
                return -1;
                                                 ...
                                                 int8 isCollision, int8 isCollision2, int8 isValid )
{
        ...
        naptParam.offset = ExtPort >> 10;
        naptParam.tcpFlag = tcpFlag;
        naptParam.isStatic = (0x1<<0) & entryType? 1: 0;
        naptParam.isValid = isValid;
# 337 "rtl8651_tblDrvPatch.c"
        return rtl8651_setAsicNaptTcpUdpTable( forced, ExtPort & (1024 -1),
                                               &naptParam );
}
                                                 ...
                                                 int8 *isDedicated, uint16 *selEIdx, uint8 *selExtIpIdx )
{
        ...
        if (extIpIdx != ((void *)0)) *extIpIdx = naptParam.insideLocalIpAddr & (16 -1);
        ...
        if ( ExtPort )
        {
                if ( naptParam.isValid == 1 && naptParam.isDedicated == 1 &&
                     (naptParam.tcpFlag&2) && (naptParam.tcpFlag&1) )
                {

                        *ExtPort = ( naptParam.offset << 10 ) + naptParam.selEIdx;
                }
                else
                {
                        *ExtPort = ( naptParam.offset << 10 ) + index;
                }
        }
        if ( tcpFlag ) *tcpFlag = naptParam.tcpFlag;
        if ( entryType )
        {
                *entryType = naptParam.isStatic ? (0x1<<0) : 0;
                *entryType |= (naptParam.isValid == 1 && naptParam.isDedicated == 1 ) ? (0x2<<0) : 0;
        }
                ...
                uint32 ageSec, int8 entryType, int16 count, int8 isCollision, int8 isValid )
{
        ...
        naptIcmp.isStatic = (0x1<<0) & entryType? 1: 0;
        naptIcmp.isValid = isValid;
        naptIcmp.offset = ExtId >> 5;
        ...
        return rtl8651_setAsicNaptIcmpTable( forced, ExtId&(32 -1), &naptIcmp );
}
                                               ...
                                               uint16 *count, int8 *isCollision, int8 *isValid )
{
        ...
        ret = rtl8651_getAsicNaptIcmpTable( tarId&(32 -1), &naptIcmp );
        if ( ret == 0 )
        {
                ...
                if ( entryType ) *entryType = naptIcmp.isStatic ?(0x1<<0): (0x0<<0);
                if ( ExtId ) *ExtId = naptIcmp.offset << 5 | (tarId&(32 -1));
        }
...
inline int32 convert_setAsicProtoTrap( uint32 index, uint8 type, uint16 content )
{
        rtl865x_tblAsicDrv_protoTrapParam_t protoTrap;

        bzero(&protoTrap, sizeof(rtl865x_tblAsicDrv_protoTrapParam_t));

        protoTrap.content = content;
        protoTrap.type = type;

        return rtl8651_setAsicProtoTrap( index, &protoTrap );
}
...
inline int32 convert_getAsicProtoTrap( uint32 index, uint8 *type, uint16 *content)
{
        ...
        ret = rtl8651_getAsicProtoTrap( index, &protoTrap );
        if ( ret != 0 ) return ret;

        if ( content ) *content = protoTrap.content;
        if ( type ) *type = protoTrap.type;
...
int32 rtl8651_setAsicL2Table_Patch(uint32 row, uint32 column, ether_addr_t * mac, int8 cpu,
        ...
        if(mac->octet[0]&0x1)
                ...
                int8 dStatic=0, dnhFlag=(isStatic==1? 1: 0);
