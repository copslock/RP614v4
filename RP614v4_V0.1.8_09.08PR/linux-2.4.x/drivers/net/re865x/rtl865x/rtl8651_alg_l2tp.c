# 1 "rtl8651_alg_l2tp.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_alg_l2tp.c"
# 68 "rtl8651_alg_l2tp.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 69 "rtl8651_alg_l2tp.c" 2
# 1 "assert.h" 1
# 70 "rtl8651_alg_l2tp.c" 2
# 1 "types.h" 1
# 71 "rtl8651_alg_l2tp.c" 2
# 1 "rtl_queue.h" 1
# 72 "rtl8651_alg_l2tp.c" 2
# 1 "rtl_errno.h" 1
# 73 "rtl8651_alg_l2tp.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
# 74 "rtl8651_alg_l2tp.c" 2

# 1 "rtl8651_alg_init.h" 1
# 122 "rtl8651_alg_init.h"
...
# 81 "rtl8651_alg_l2tp.c" 2

static uint32 last_ip=0;
static uint32 session_cnt=0;
static struct l2tp_session *session=((void *)0);


int32 rtl8651_l4AliasL2tpInit(void *arg)
{
        if(session) rtlglue_free(session);
        session = (struct l2tp_session *)rtlglue_malloc(sizeof(struct l2tp_session)*algMaxSession.l2tpMaxSession);
        memset((void*)session,0,sizeof(struct l2tp_session)*algMaxSession.l2tpMaxSession);
        last_ip=0;
        session_cnt=0;
        return 0;
}
...
int32 rtl8651_l4AliasHandleL2tpClientOutbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        struct udphdr *udp;
        int32 tlen;
        struct l2tp_header *l2tp;




        if((alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000004)==0) return 131;



        udp = (struct udphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));

        if(udp->uh_sport!=udp->uh_dport) return 0;

        tlen = (pip->ip_len);

        if(tlen-36<0) return 0;


        l2tp = (struct l2tp_header *)(udp+1);

        if(last_ip==0) last_ip=((uint32) (*((uint8*)(&pip->ip_src.s_addr))<<24)|(*(((uint8*)(&pip->ip_src.s_addr))+1)<<16)|(*(((uint8*)(&pip->ip_src.s_addr))+2)<<8) | *(((uint8*)(&pip->ip_src.s_addr))+3));



        if(tb->insideLocalPort==1701)
        {
                rtl8651_addUpnpMap(0x04, tb->dstIpAddr, 1701,
                        tb->insideGlobalIpAddr , 1701,
                        tb->insideLocalIpAddr , 1701);
                        tb->insideGlobalPort=1701;
        }


        if(((l2tp->flag)&0xf4ff)==0xc002)
        {
                if((l2tp->session_id==0)&&(l2tp->tunnel_id==0))
                {
                        last_ip=((uint32) (*((uint8*)(&pip->ip_src.s_addr))<<24)|(*(((uint8*)(&pip->ip_src.s_addr))+1)<<16)|(*(((uint8*)(&pip->ip_src.s_addr))+2)<<8) | *(((uint8*)(&pip->ip_src.s_addr))+3));
...
int32 rtl8651_l4AliasHandleL2tpClientInbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        ...
        for(i=0;i<algMaxSession.l2tpMaxSession;i++)
        {
                if((session[i].tunnel_id==l2tp->tunnel_id)&&(l2tp->tunnel_id!=0))
                {
                        tb->insideLocalIpAddr=session[i].redirect_ip;
        ...
        if(((l2tp->flag)&0xf4ff)==0xc002)
        {
                for(i=0;i<algMaxSession.l2tpMaxSession;i++)
                {
                        ...
                        {
                                session[i].tunnel_id=l2tp->tunnel_id;

                                break;
                        }
                }

                if(i==algMaxSession.l2tpMaxSession)
                {
                        session[session_cnt].tunnel_id=l2tp->tunnel_id;
                        session[session_cnt].redirect_ip=last_ip;

                        ++session_cnt;
                        session_cnt%=algMaxSession.l2tpMaxSession;
                }
        }

        tb->insideLocalIpAddr=last_ip;
        return 0;
}
