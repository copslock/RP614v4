...
* Abstract : 
* $Id: voip_support.h,v 1.5 2005/06/06 06:34:25 yjlou Exp $
*
*/
...
#define VOIP_CATEGORY( chid, mid ) (VOIP_CATEGORY_BASE+(chid<<2)+mid)
#define VOIP_CATEGORY_MAP( cat, pChid, pMid ) \
	do { \
		if ( VOIP_CATEGORY_VALID( cat ) ) \
		{ \
			*(pChid) = ((cat)-VOIP_CATEGORY_BASE)>>2; \
			*(pMid) = ((cat)-VOIP_CATEGORY_BASE)&0x3; \
		} \
	} while(0)
