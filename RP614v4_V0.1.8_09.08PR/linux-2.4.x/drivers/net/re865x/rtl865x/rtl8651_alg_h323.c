# 1 "rtl8651_alg_h323.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_alg_h323.c"
# 169 "rtl8651_alg_h323.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 170 "rtl8651_alg_h323.c" 2
# 1 "assert.h" 1
# 171 "rtl8651_alg_h323.c" 2
# 1 "types.h" 1
# 172 "rtl8651_alg_h323.c" 2
# 1 "rtl_queue.h" 1
# 173 "rtl8651_alg_h323.c" 2
# 1 "rtl_errno.h" 1
# 174 "rtl8651_alg_h323.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
static uint8 *_rtl8651_l4_h323_alias_buf[2];

static int32 init_h323 = 0;
static uint16 tempPort = 0;

static uint16 lastPort[4]={0};
static uint32 RsvpLocalIp = 0;
static uint32 RsvpGlobalIp = 0;


static int32 find_h245_id(uint8 *data, int32 datlen);
static int32 find_port (int32, uint8 *, int32 , int32 *, uint16 *);
# 360 "rtl8651_alg_h323.c"
static int32 find_port(ipaddr, data, datlen, off, port)
int32 ipaddr;
uint8 *data;
int32 datlen, *off;
uint16 *port;
...
static int32 find_h245_id(uint8 *data, int32 datlen)
{
        uint8 h245_id[] = { 0x08, 0x81, 0x75, 0x00 };

        uint32 id1, id2;
        uint8 *dp;
        int32 offset;

        if (datlen < 5 || data == ((void *)0))
                return 0;

        offset = 0;
        dp = (uint8 *)data;
        id1 = (h245_id[0] << 24) | (h245_id[1] << 16) | (h245_id[2] << 8) | h245_id[3];

        for (offset = 0; offset <= datlen - 5; offset++, dp++) {
                id2 = (dp[0] << 24) | (dp[1] << 16) | (dp[2] << 8) | dp[3];
                if (id1 == id2 )
                {
                        return 1;
                }
...
# 441 "rtl8651_alg_h323.c"
int32 rtl8651_l4AliasHandleClientH225RASIn (
                                        struct rtl_pktHdr* pkthdrPtr,
                                        struct ip *iphdr,
                                        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link)
{
        struct rtl_mBuf *mbuf = pkthdrPtr->PKTHDRNXT.mbuf_first;
        struct udphdr *ud = ((void *)0);
        struct ip *pip = ((void *)0);
        uint8 *cp = ((void *)0), *cp_end = ((void *)0), *data = ((void *)0);
        uint32 mins = mbuf->m_len, copied = 0, datalen = 0, hlen = 0, chksum_flag = 0;
        int32 offset = 0;
        uint16 port = 0;
        ...
        if ( (alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000010) == 0 )
        {
                goto out;
        }
        ...
        cp = _rtl8651_l4_h323_alias_buf[0];

        copied = (int32)mBuf_copyToUserBuffer( mbuf,
                                                                                        0,
                                                                                        mins,
                                                                                        (int8 *)(_rtl8651_l4_h323_alias_buf[0]));
        cp_end = cp + copied;

        pip = (struct ip *)(&_rtl8651_l4_h323_alias_buf[0][pkthdrPtr->ph_iphdrOffset]);

        switch (pip->ip_p)
        {
                case 17:
                        ud = (struct udphdr *)((uint8 *)pip + (((*(uint8*)pip)&0xf) << 2));
                        hlen = ((pip->ip_vhl & 0xf) + sizeof(struct udphdr));
                        break;
                default:
                        do {} while (0);
                        return 0;
        ...
        data = &_rtl8651_l4_h323_alias_buf[0][pkthdrPtr->ph_iphdrOffset + hlen];
        datalen = copied - hlen;

        while (find_port(link->insideGlobalIpAddr, data, datalen, &offset, &port) == 0 )
        {
                ...
                do{ *((uint8*)((uint8*)&(data[offset]))) = (uint8)((link->insideLocalIpAddr)>>24); *(((uint8*)((uint8*)&(data[offset])))+1) = (uint8)((link->insideLocalIpAddr)>>16); *(((uint8*)((uint8*)&(data[offset])))+2) = (uint8)((link->insideLocalIpAddr)>>8); *(((uint8*)((uint8*)&(data[offset])))+3) = (uint8)((link->insideLocalIpAddr)); }while(0);
        ...
        mBuf_copyToMbuf(mbuf, 0,copied,(int8 *)_rtl8651_l4_h323_alias_buf[0]);

out:
        return 0;
}

int32 rtl8651_l4AliasHandleClientH225RASOut (
                                        struct rtl_pktHdr* pkthdrPtr,
                                        struct ip *iphdr,
                                        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link)
{
        ...
        while (find_port (link->insideLocalIpAddr, data, datalen, &offset, &port) == 0)
        {
                chksum_flag = 1;

                if (port == link->insideGlobalPort)
                {
# 645 "rtl8651_alg_h323.c"
                } else if (port == link->insideLocalPort)
                {
# 655 "rtl8651_alg_h323.c"
                        data[offset + sizeof(ipaddr_t)] = ((link->insideGlobalPort)) >> 8;
                        data[offset + sizeof(ipaddr_t) + 1] = ((link->insideGlobalPort)) & 0xff;
                } else
                {
                        ...
                        uint32 retval;
# 676 "rtl8651_alg_h323.c"
                        if ((retval = _rtl8651_addNaptConnection( 1, (1|0x0002),
                                                                                                        1,
                                                                                                        link->insideLocalIpAddr, port,
                                                                                                        &link->insideGlobalIpAddr,&port,
                                                                                                        0, 0)) != 0)
                        {
                                if (retval != -3005)
                                {
                ...
                do{ *((uint8*)(((uint8*)&(data[offset])))) = (uint8)((link->insideGlobalIpAddr)>>24); *(((uint8*)(((uint8*)&(data[offset]))))+1) = (uint8)((link->insideGlobalIpAddr)>>16); *(((uint8*)(((uint8*)&(data[offset]))))+2) = (uint8)((link->insideGlobalIpAddr)>>8); *(((uint8*)(((uint8*)&(data[offset]))))+3) = (uint8)((link->insideGlobalIpAddr)); }while(0);
...
int32 rtl8651_l4AliasHandleClientH225CSIn(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
        ...
        if((alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000010)==0) return 0;

        m = (struct rtl_mBuf *)pkthdrPtr->PKTHDRNXT.mbuf_first;
        mins=m->m_len;



        ipaddr.s_addr = ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3));
        ...
        if(mins>576) mins=576;
        cp = _rtl8651_l4_h323_alias_buf[0];
        copied = (int32)mBuf_copyToUserBuffer((struct rtl_mBuf *)m, (uint32)0, mins, (int8 *)(_rtl8651_l4_h323_alias_buf[0]));
        cp_end = cp + copied;


        pip = (struct ip *)(&_rtl8651_l4_h323_alias_buf[0][pkthdrPtr->ph_iphdrOffset]);
        tcp = (struct tcphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));

        hlen = ((pip->ip_vhl & 0xf) + (( tcp->th_off_x >>4)&0xf)) << 2 ;


        data = ((uint8 *)pip)+hlen;

        datalen = copied - hlen;
# 791 "rtl8651_alg_h323.c"
        if( link->h245tunnel ||
                (find_h245_id(data,datalen)) ){
        ...
        if (find_port(ipaddr.s_addr, data, datalen, &off, &port) == 0) {

        {
                ...
                addr_1 = link->insideGlobalIpAddr;
                port_1 = link->insideLocalPort+1;
                        ...
                        r = _rtl8651_addNaptConnection(1,(1|0x0002), 1,
                                link->insideLocalIpAddr,link->insideLocalPort+1,
                                &addr_1,&port_1,
                                link->dstIpAddr,port);
                ...
                if(_rtl8651_getOutboundNaptFlow(1,link->insideLocalIpAddr,link->insideLocalPort+1,
                                link->dstIpAddr,port,&tb)!=0)
                        {

                                goto end_proc;
                        }

                        tb->alg_client_out = rtl8651_l4AliasHandleClientH245Out;
                        tb->alg_client_in = rtl8651_l4AliasHandleClientH245In;
...
end_proc:
        ...
        while( find_port(link->insideGlobalIpAddr, data, datalen, &off, &port) ==0 ) {




                chksum_flag = 1;
                data[off]=(link->insideLocalIpAddr>>24)&0xff;
                data[off+1]=(link->insideLocalIpAddr>>16)&0xff;
                data[off+2]=(link->insideLocalIpAddr>>8)&0xff;
                data[off+3]=(link->insideLocalIpAddr)&0xff;
        ...
        if(chksum_flag){
                ...
                if((pip->ip_vhl & 0xf)!=5)
                {
                        tcp->th_sum = 0;
                        pip->ip_sum = 0;
                        tcp->th_sum = _rtl8651_tcpChecksum(pip);
                        pip->ip_sum = _rtl8651_ipChecksum(pip);
                }
        }

        mBuf_copyToMbuf(m, 0,copied,(int8 *)_rtl8651_l4_h323_alias_buf[0]);
...
int32 rtl8651_l4AliasHandleClientH225CSOut(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
        ...
        uint8 *cp;
        uint8 *cp_end;
        struct rtl_mBuf *m = pkthdrPtr->PKTHDRNXT.mbuf_first;
        int32 hlen,datalen;
        int32 copied = 0;
        int32 off=0;
        uint16 result_port=0;
        uint8 chksum_flag=0;
        uint32 mins=m->m_len;
        ...
        tcp = (struct tcphdr *)(struct tcphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));

        data = (uint8 *)tcp + ((tcp->th_off_x & 0xf0) >> 2);
        tlen = (pip->ip_len);
        ...
        while( find_port(link->insideLocalIpAddr, data, datalen, &off, &result_port) ==0 ) {



                chksum_flag = 1;
                data[off]=(link->insideGlobalIpAddr>>24)&0xff;
                data[off+1]=(link->insideGlobalIpAddr>>16)&0xff;
                data[off+2]=(link->insideGlobalIpAddr>>8)&0xff;
                data[off+3]=(link->insideGlobalIpAddr)&0xff;
        ...
        data = &_rtl8651_l4_h323_alias_buf[0][pkthdrPtr->ph_iphdrOffset+hlen];
        datalen = copied - hlen;
        while( find_port(link->insideGlobalIpAddr, data, datalen, &off, &result_port) ==0 ) {


                _rtl8651_addAlgQosNaptConnection(1, 1|0x0080|0x0100,0, link->insideLocalIpAddr, result_port, &link->insideGlobalIpAddr, &result_port,0,0,4);
        ...
        if( link->h245tunnel ||
                find_h245_id(cp,cp_end-cp) ){


                link->h245tunnel = 1;

                rtl8651_l4AliasHandleClientH245Out(pkthdrPtr,pip,link);
        }
        ...
        RsvpLocalIp = link->insideLocalIpAddr;
        RsvpGlobalIp = link->insideGlobalIpAddr;

        return 0;
}


int32 rtl8651_l4AliasHandleClientH245In(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
# 1034 "rtl8651_alg_h323.c"
        return 0;

}


int32 rtl8651_l4AliasHandleClientH245Out(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
        struct in_addr ipaddr_org;
        int32 off=0, datlen;
        uint16 result_port;
        uint8 *data = ((void *)0);
        struct tcphdr *tcp = ((void *)0);
        int32 hlen;
        int32 chksum_flag = 0;
        int32 copied = 0;
        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s entry, *tb = &entry;
        ...
        m = pkthdrPtr->PKTHDRNXT.mbuf_first;
        mins=m->m_len;
        if(mins>576) mins=576;
        copied = (int32)mBuf_copyToUserBuffer((struct rtl_mBuf *)m, (uint32)0, mins, (int8 *)(_rtl8651_l4_h323_alias_buf[0]));
        pip = (struct ip *)(&_rtl8651_l4_h323_alias_buf[0][pkthdrPtr->ph_iphdrOffset]);
        ...
        ipaddr_org.s_addr = link->insideLocalIpAddr;

        while( find_port(ipaddr_org.s_addr, data, datlen, &off, &result_port) ==0 ) {
                ...
                if ( result_port == 1503 ) goto end_aliash245Out;

                if(link->natlink == 0){

                        if(_rtl8651_getOutboundNaptFlow(0,link->insideLocalIpAddr,result_port,link->dstIpAddr,lastPort[portIdx], &tb)==0){
                                _rtl8651_delNaptConnection(tb);
                        }
                                ...
                                _rtl8651_removeNaptConnectionByIntIPandPort(0,link->insideLocalIpAddr,result_port);

                                _rtl8651_addAlgQosNaptConnection(1, 1|0x0080,0, link->insideLocalIpAddr, result_port, &link->insideGlobalIpAddr, &result_port,0,0,4);
# 1107 "rtl8651_alg_h323.c"
                        portIdx++;
                }
...
static int32 _rtl8651_l4RsvpNaptAliasIn(struct rtl_pktHdr *pkthdrPtr,struct ip * pip)
{
        int32 retval = 35;
        uint8 *data=((void *)0),*dp;
        uint32 dip,addr,offset;
        uint16 datlen,port;
        ...
        data = (uint8*)((uint8 *) pip + ((pip->ip_vhl &0xf) << 2));
        datlen = (uint32)data-(uint32)pip;
        dp = data;

        for (offset = 0; offset < datlen - 8; offset++, dp++)
        {
                addr = (dp[0] << 24) | (dp[1] << 16) | (dp[2] << 8) | dp[3];
                if (dip == addr)
                {
                        port = (uint16)((*(dp + 6) << 8) | *(dp + 7));
                        if ( _rtl8651_addAlgQosNaptConnection( 1,
                                                                                                        ...
                                                                                                        4) == 0)
                        {
                                do{ *((uint8*)(&(pip->ip_dst))) = (uint8)((RsvpLocalIp)>>24); *(((uint8*)(&(pip->ip_dst)))+1) = (uint8)((RsvpLocalIp)>>16); *(((uint8*)(&(pip->ip_dst)))+2) = (uint8)((RsvpLocalIp)>>8); *(((uint8*)(&(pip->ip_dst)))+3) = (uint8)((RsvpLocalIp)); }while(0);
                                retval = 0;
...
static int32 _rtl8651_l4RsvpNaptAliasOut(struct rtl_pktHdr *phdr,struct ip * pip)
{

        if ((alg_supports[phdr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000010) == 0 )
        {
                return 35;
        }
        ...
        if (RsvpGlobalIp != 0)
        {
                do{ *((uint8*)(&pip->ip_src)) = (uint8)((RsvpGlobalIp)>>24); *(((uint8*)(&pip->ip_src))+1) = (uint8)((RsvpGlobalIp)>>16); *(((uint8*)(&pip->ip_src))+2) = (uint8)((RsvpGlobalIp)>>8); *(((uint8*)(&pip->ip_src))+3) = (uint8)((RsvpGlobalIp)); }while(0);
                return 0;
        }
...
void rtl8651_l4AliasH323ReInit(void)
{
        ...
        rtl8651_l4RsvpNaptAliasIn = _rtl8651_l4RsvpNaptAliasIn;
        rtl8651_l4RsvpNaptAliasOut = _rtl8651_l4RsvpNaptAliasOut;

}

int32 rtl8651_l4AliasH323Init(void *arg)
{
        ...
        init_h323 = 1;
        _rtl8651_l4_h323_alias_buf[0] = (uint8 *)rtlglue_malloc(576 +64);
        _rtl8651_l4_h323_alias_buf[1] = (uint8 *)rtlglue_malloc(576 +64);
        do {} while (0);
        do {} while (0);

        memset(_rtl8651_l4_h323_alias_buf[0], 0, sizeof(576 +64));
        memset(_rtl8651_l4_h323_alias_buf[1], 0, sizeof(576 +64));
...
int32 rtl8651_l4AliasHandleServerH225RASIn (
                                        struct rtl_pktHdr* pkthdrPtr,
                                        struct ip *pip,
                                        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link)
{
...
int32 rtl8651_l4AliasHandleServerH225RASOut (
                                        struct rtl_pktHdr* pkthdrPtr,
                                        struct ip *pip,
                                        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link)
{
...
int32 rtl8651_l4AliasHandleServerH225CSOut(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{

        struct tcphdr *tcp;
        int32 tlen,hlen,dlen;
        struct rtl_mBuf *m = pkthdrPtr->PKTHDRNXT.mbuf_first;
        int32 copied = 0;
        int off;
        uint16 result_port;
        ...
        copied = mBuf_copyToUserBuffer(m, 0,
                mins,
                (int8 *)_rtl8651_l4_h323_alias_buf[1]);


        pip = (struct ip *)(&_rtl8651_l4_h323_alias_buf[1][pkthdrPtr->ph_iphdrOffset]);
        tcp = (struct tcphdr *) ((int8 *) pip + ((pip->ip_vhl &0xf) << 2));
        hlen = ((pip->ip_vhl & 0xf) + (( tcp->th_off_x >>4)&0xf)) << 2 ;

        data = &_rtl8651_l4_h323_alias_buf[1][pkthdrPtr->ph_iphdrOffset+hlen];
        datalen = copied - hlen;
        ...
        while(find_port(link->insideLocalIpAddr, data, datalen, &off, &result_port) ==0)
        {
                ...
                tempPort=result_port;


                rtl8651_addUpnpMap(0x04|0x02 , link->dstIpAddr, 0,
                        link->insideGlobalIpAddr, 17200,
                        link->insideLocalIpAddr, result_port);



                result_port=17200;
                data[off+4]=(result_port>>8)&0xff;
                data[off+5]=(result_port)&0xff;
                chksum_flag=1;
                data+=(off+6);
                datalen-=(off+6);
                off = 0;
        ...
        mBuf_copyToMbuf(m, 0,copied,(int8 *)_rtl8651_l4_h323_alias_buf[1]);

        return 0;

}

int32 rtl8651_l4AliasHandleServerH225CSIn(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
...
int32 rtl8651_l4AliasHandleServerH245Out(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
        ...
        while(find_port(link->insideLocalIpAddr, &data[offset], mins-pkthdrPtr->ph_iphdrOffset-hlen-offset, &off, &result_port) ==0)
        {
                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s entry, *tb = &entry;
                data[off+offset]=(link->insideGlobalIpAddr>>24)&0xff;
                data[1+off+offset]=(link->insideGlobalIpAddr>>16)&0xff;
                data[2+off+offset]=(link->insideGlobalIpAddr>>8)&0xff;
                data[3+off+offset]=(link->insideGlobalIpAddr)&0xff;



                if(result_port==1503) goto H245Out_break;
...
int32 rtl8651_l4AliasHandleServerH245In(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                                struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *link)
{
# 1572 "rtl8651_alg_h323.c"
        return 0;
