...
# 9 "rtl8651_ipQueue.c" 2

# 1 "rtl8651_ipQueue.h" 1
# 9 "rtl8651_ipQueue.h"
...
# 15 "rtl8651_ipQueue.c" 2
# 32 "rtl8651_ipQueue.c"
static struct ipq *ipq_hash[64];
static uint32 ipfrag_hash_rnd=31;


static struct ipq *negative_ipq[64];


static struct ipq *positive_ipq[64];
...
static struct ipq *lru_list = ((void *)0);
static struct ipq *lru_negative_list = ((void *)0);
static struct ipq *lru_positive_list = ((void *)0);





static uint32 sysctl_ipfrag_time = 3;
static uint32 negative_list_time = 1;
static uint32 rtl8651_drvMaxNegativeListCnt = 20;
static uint32 rtl8651_drvMaxPositiveListCnt = 60;
static uint32 rtl8651_drvMaxFragSubPktCnt = 12;
static uint32 rtl8651_drvMaxFragPoolCnt = 60;
static uint32 rtl8651_drvMaxFragPktCnt = 10;
static uint32 negative_list_cnt = 0;
static uint32 positive_list_cnt = 0;
static uint32 frag_flow_cnt = 0;
static uint32 frag_pkt_cnt = 0;
static uint32 ip_frag_mem = 0;
...
struct ipq *ipq_pool;
static struct ipq* freelist;


extern uint16 *_rtl8651_udpSizeThreshValue;
extern uint8 _rtl8651_fragQueueProcess;
extern uint8 _rtl8651_fragQueueDropChain;
extern int8 rtl8651_drvIcmpRoutingMsgEnable;

extern ipaddr_t _rtl8651_fragQueueNaptRedirect_MatchedIP;
...
# 125 "rtl8651_ipQueue.c"
static __inline__ unsigned int ipqhashfn(uint16, uint32, uint32, uint8);
static __inline__ struct ipq *ipq_alloc(void);
static __inline__ void ipq_link(struct ipq *, unsigned int);
static __inline__ void ipq_unlink(struct ipq *);
static __inline__ void ipq_free(struct ipq *);
static __inline__ void frag_freepktMbuf(struct rtl_pktHdr *);
static __inline__ void frag_freeMbufChain(struct ipq *);
static __inline__ void negative_unlink(struct ipq *);
static __inline__ void negative_free(struct ipq *);
static void frag_delNegative(struct ipq *);
static void frag_ipq2Negative(struct ipq *);
static __inline__ void positive_unlink(struct ipq *);
static __inline__ void positive_free(struct ipq *);
static void frag_delPositive(struct ipq *);
static void frag_ipq2Positive(struct ipq *);
static void frag_destroy(struct ipq *);
static void frag_tryDelNegative(void);
static int32 frag_tryExpire(void);
static void frag_expire(unsigned long);
static struct ipq *ip_frag_create(unsigned int, struct ip *);
static struct ipq *ip_frag_find(struct ip *);
int32 ip_frag_queue(struct ipq *, struct rtl_pktHdr *,struct ip *);
static int32 _rtl8651_flushFragQueue(struct rtl_pktHdr *, struct rtl_pktHdr *);
static __inline__ void _rtl8651_IpFragQueueSetInitPara(rtl8651_fwdEngineInitPara_t *para);
static __inline__ void ipqPool_init(void);
static __inline__ void ipqPool_reinit(void);
static void * ipqPool_malloc(uint32);
static void ipqPool_free(void*);
# 162 "rtl8651_ipQueue.c"
void rtl8651_IpFragQueueSetTimeout(_rtl8651_IpFragQueueTimeout_t *para)
{
        if (!para)
                return;
        ...
        if (para->MaxFragTimeout)
        {
                sysctl_ipfrag_time = para->MaxFragTimeout;
        }
        if (para->MaxNegativeListTimeOut)
        {
                negative_list_time = para->MaxNegativeListTimeOut;
        }
        rtlglue_drvMutexUnlock();
}
void _rtl8651_IpFragQueueSetInitPara(rtl8651_fwdEngineInitPara_t *para)
{
        if (!para)
                return;

        rtl8651_drvMaxFragPktCnt = para->MaxFragPktCnt;
        rtl8651_drvMaxFragSubPktCnt = para->MaxFragSubPktCnt;
        rtl8651_drvMaxNegativeListCnt = para->MaxNegativeListEntryCnt;
        rtl8651_drvMaxPositiveListCnt = para->MaxPositiveListEntryCnt;
        rtl8651_drvMaxFragPoolCnt = para->MaxFragPoolCnt;
        ...
        if ((!rtl8651_drvMaxFragPktCnt) ||
                ((!rtl8651_drvMaxFragSubPktCnt) && (!rtl8651_drvMaxFragPoolCnt)))
        {
                system_enable = 0;
        }
}
...
int32 rtl8651_IpFragQueueReinit(void)
{
        ...
        for (index = 0 ; index < 64 ; index ++)
        {
                ...
                while (ipq_hash[index])
                {
                        frag_destroy(ipq_hash[index]);
                }

                while(negative_ipq[index])
                {
                        frag_delNegative(negative_ipq[index]);
                }

                while(positive_ipq[index])
                {
                        frag_delPositive(positive_ipq[index]);
                }
        }
...
int32 _rtl8651_IpFragQueueInit(rtl8651_fwdEngineInitPara_t *para)
{
        if (para)
        {
                _rtl8651_IpFragQueueSetInitPara(para);
        }
        if (system_enable == 0)
        {
                return 0;
...
# 307 "rtl8651_ipQueue.c"
static __inline__ unsigned int ipqhashfn(uint16 id, uint32 saddr, uint32 daddr, uint8 prot)
{
        return jhash_3words((uint32)id << 16 | prot, saddr, daddr,
                            ipfrag_hash_rnd) & (64 - 1);
}



void ipqPool_init(void)
{
        rtlglue_drvMutexLock();

        ipq_pool = (struct ipq *)rtlglue_malloc(sizeof(struct ipq) * (rtl8651_drvMaxFragPktCnt + rtl8651_drvMaxNegativeListCnt + rtl8651_drvMaxPositiveListCnt));
        freelist = ipq_pool;

        rtlglue_drvMutexUnlock();
}
void ipqPool_reinit(void)
{
        uint32 idx;
        struct ipq *ptr;

        if ((rtl8651_drvMaxFragPktCnt + rtl8651_drvMaxNegativeListCnt + rtl8651_drvMaxPositiveListCnt) == 0)
        {
                freelist = ((void *)0);
                return;
        }
        ...
        memset(ipq_pool, 0, (sizeof(struct ipq) * (rtl8651_drvMaxFragPktCnt + rtl8651_drvMaxNegativeListCnt + rtl8651_drvMaxPositiveListCnt)));
        ...
        for (idx = 0 ; idx < (rtl8651_drvMaxFragPktCnt + rtl8651_drvMaxNegativeListCnt + rtl8651_drvMaxPositiveListCnt) ; idx ++, ptr ++)
        {



                if (idx == ((rtl8651_drvMaxFragPktCnt + rtl8651_drvMaxNegativeListCnt + rtl8651_drvMaxPositiveListCnt) - 1))
                {
                        ptr->next = ((void *)0);
                }else
                {
...
void * ipqPool_malloc(uint32 ipq_sz)
{
        struct ipq *ret;
...
void ipqPool_free(void* ipq)
{
        memset(ipq, 0, sizeof(struct ipq));
        ((struct ipq *)ipq)->next = freelist;
        freelist = ipq;
}
...
static __inline__ struct ipq *ipq_alloc(void)
{
        ...
        qp = (struct ipq*)ipqPool_malloc(sizeof(struct ipq));
...
static __inline__ void ipq_link(struct ipq *qp, unsigned int hash)
{
        struct ipq *orig;


        _rtl8651_init_timer(&qp->timer);
        qp->timer.data = (uint32) qp;
        qp->timer.function =(void*)frag_expire;
        _rtl8651_mod_timer(&qp->timer, _sysUpSeconds + sysctl_ipfrag_time);
...
static __inline__ void ipq_unlink(struct ipq *qp)
{

        _rtl8651_del_timer(&qp->timer);
        ...
        if (qp->next_lru)
        {
                qp->next_lru->prev_lru = qp->prev_lru;
        }
        if (qp->prev_lru)
        {
                qp->prev_lru->next_lru = qp->next_lru;
        }
        if (lru_list == qp)
        {
                lru_list = qp->next_lru;
        }
        qp->next_lru = ((void *)0);
        qp->prev_lru = ((void *)0);
...
static __inline__ void ipq_free(struct ipq *qp)
{
...
static __inline__ void frag_freepktMbuf(struct rtl_pktHdr *pkt)
{
        frag_pkt_cnt --;
        ip_frag_mem -= pkt->PKTHDRNXT.mbuf_first->m_extsize;
        pkt->ph_nextHdr = ((void *)0);
        mBuf_freeMbufChain(pkt->PKTHDRNXT.mbuf_first);
}



static __inline__ void frag_freeMbufChain(struct ipq *qp)
{
        struct rtl_pktHdr *head, *fp;

        if ((head = qp->fragments)!=((void *)0))
        {
                for ( ; head ; )
                {
                        fp = head->ph_nextHdr;
                        frag_freepktMbuf(head);
                        head = fp;
                }
        }
        return;
...
void frag_destroy(struct ipq *qp)
{

        ipq_unlink(qp);


        frag_freeMbufChain(qp);


        frag_ipq2Negative(qp);
...
int32 frag_tryExpire(void)
{
        uint8 expireOK = 0;

        while (lru_list)
        {
                if (_sysUpSeconds >= lru_list->timer.expires)
                {
                        frag_expire((unsigned long)lru_list);
                        expireOK = 1;
                }else
                {
                        break;
...
void frag_expire(unsigned long arg)
{
        ...
        if (rtl8651_drvIcmpRoutingMsgEnable)
        {
                ...
                struct rtl_pktHdr *pkthdr = qp->fragments;
                struct ip *pip = (struct ip *)((uint8 *)(pkthdr->PKTHDRNXT.mbuf_first->m_data) + (pkthdr->ph_iphdrOffset));
                rtl8651_tblDrv_vlanTable_t *vlanp = &DrvTbl.vlan[pkthdr->ph_vlanIdx & (8 - 1)];
                uint16 vid = vlanp->vid;
                ...
                qp->fragments = pkthdr->ph_nextHdr;
                pkthdr->ph_nextHdr = ((void *)0);
                ip_frag_mem -= pkthdr->PKTHDRNXT.mbuf_first->m_extsize;
                frag_pkt_cnt --;

                if (_rtl8651_drvIcmpErrorGeneration(pkthdr, pip, &icmp_info, vid) != 0)
                {
...
struct ipq *ip_frag_create(unsigned int hash, struct ip *pip)
{
        struct ipq *qp;
        struct ipq *lru;
        struct ipq *ipq_recycle;



        if ((frag_flow_cnt >= rtl8651_drvMaxFragPktCnt) &&
                (frag_tryExpire() != 0))
        {
                goto out_err;
        }
        ...
        if ((qp = ipq_alloc()) == ((void *)0))
        {
                goto out_nomem;
        }


        qp->protocol = pip->ip_p;
        qp->id = (pip->ip_id);
        qp->saddr = ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3));
        qp->daddr = ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3));
        qp->last_in = 0;
        qp->len = 0;
        qp->meat = 0;
        qp->fragments = ((void *)0);
...
out_err:
        ...
        if ((ipq_recycle = ipqPool_malloc(sizeof(struct ipq))) != ((void *)0))
        {
                ip_frag_mem += sizeof(struct ipq);
                goto add_negative;
        }
...
out_nomem:
        ...
        if (lru_negative_list)
        {
                ipq_recycle = lru_negative_list;

                negative_unlink(ipq_recycle);
                negative_list_cnt --;
        }else
        {
                goto out;
...
add_negative:
                ...
                ipq_recycle->protocol = pip->ip_p;
                ipq_recycle->id = (pip->ip_id);
                ipq_recycle->saddr = ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3));
                ipq_recycle->daddr = ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3));
                ipq_recycle->last_in = 0;
                ipq_recycle->len = 0;
                ipq_recycle->meat = 0;
                ipq_recycle->fragments = ((void *)0);
                ipq_recycle->subPktCnt = 0;
                ipq_recycle->hash_idx = hash;
                ipq_recycle->prev_lru = ((void *)0);
                ipq_recycle->next_lru = ((void *)0);


                frag_ipq2Negative(ipq_recycle);
...
static __inline__ void positive_unlink(struct ipq *qp)
{
        ...
        if (lru_positive_list == qp)
        {
                lru_positive_list = qp->next_lru;
        }
        qp->next_lru = ((void *)0);
        qp->prev_lru = ((void *)0);
}
...
static __inline__ void positive_free(struct ipq *qp)
{
...
void frag_delPositive(struct ipq *qp)
{
...
void frag_ipq2Positive(struct ipq *qp)
{
        unsigned int hash;
        struct ipq *orig;
        struct ipq *lru_positive;


        if (positive_list_cnt >= rtl8651_drvMaxPositiveListCnt)
        {
                if (lru_positive_list)
                {
                        frag_delPositive(lru_positive_list);
                } else
                {
        ...
        hash = qp->hash_idx;
        orig = positive_ipq[hash];
        qp->next = orig;
        qp->pprev = &positive_ipq[hash];
        if (orig)
        {
                orig->pprev = &(qp->next);
        }
        positive_ipq[hash] = qp;
        ...
        if (!lru_positive_list)
        {
                lru_positive_list = qp;
        }else
        {
                lru_positive = lru_positive_list;
                while(lru_positive->next_lru)
                {
                        lru_positive = lru_positive->next_lru;
                }
                lru_positive->next_lru = qp;
                qp->prev_lru = lru_positive;
        }
        positive_list_cnt ++;
...
static __inline__ void negative_unlink(struct ipq *qp)
{
        _rtl8651_del_timer(&qp->timer);
        ...
        if (lru_negative_list == qp)
        {
                lru_negative_list = qp->next_lru;
        }
        qp->next_lru = ((void *)0);
        qp->prev_lru = ((void *)0);
}
...
static __inline__ void negative_free(struct ipq *qp)
{
...
void frag_delNegative(struct ipq *qp)
{
...
void frag_ipq2Negative(struct ipq *qp)
{
        unsigned int hash;
        struct ipq *orig;
        struct ipq *lru_negative;


        if (negative_list_cnt >= rtl8651_drvMaxNegativeListCnt)
        {
                if (lru_negative_list)
                {
                        frag_delNegative(lru_negative_list);
                } else
                {
        ...
        _rtl8651_init_timer(&qp->timer);
        qp->timer.data = (uint32) qp;
        qp->timer.function =(void*)frag_delNegative;
        _rtl8651_mod_timer(&qp->timer, _sysUpSeconds + negative_list_time);


        hash = qp->hash_idx;
        orig = negative_ipq[hash];
        qp->next = orig;
        qp->pprev = &negative_ipq[hash];
        if (orig)
        {
                orig->pprev = &(qp->next);
        }
        negative_ipq[hash] = qp;
        ...
        if (!lru_negative_list)
        {
                lru_negative_list = qp;
        }else
        {
                lru_negative = lru_negative_list;
                while(lru_negative->next_lru)
                {
                        lru_negative = lru_negative->next_lru;
                }
                lru_negative->next_lru = qp;
                qp->prev_lru = lru_negative;
        }
...
void frag_tryDelNegative(void)
{
        while (lru_negative_list)
        {
                if (_sysUpSeconds >= lru_negative_list->timer.expires)
                {
                        frag_delNegative(lru_negative_list);
                }else
                {
...
struct ipq *ip_frag_find(struct ip *pip)
{

        uint16 id = (pip->ip_id);
        uint32 saddr = ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3));
        uint32 daddr = ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3));
        uint8 protocol = pip->ip_p;
        ...
        if (positive_ipq[hash] == ((void *)0))
        {
                goto check_negative;
        }

        for (qp = positive_ipq[hash]; qp; qp = qp->next)
        {
                if(qp->id == id &&
                   qp->saddr == saddr &&
                   qp->daddr == daddr &&
...
check_negative:
        ...
        if (negative_ipq[hash] == ((void *)0))
        {
                goto check_incomplete;
        }

        for (qp = negative_ipq[hash]; qp; qp = qp->next)
        {
                if(qp->id == id &&
                   qp->saddr == saddr &&
                   qp->daddr == daddr &&
...
new_one:
        return ip_frag_create(hash, pip);
}
# 1044 "rtl8651_ipQueue.c"
int32 ip_frag_queue(struct ipq *qp, struct rtl_pktHdr *pktHdrPtr,struct ip * ip)
{
        struct rtl_pktHdr *prev, *next;
        int flags, offset;
        int ihl, end;
        int32 retval;
        rtl8651_tblDrv_vlanTable_t *vlanp = ((void *)0);
        uint8 direction = 0;
        uint32 dosIgnore = 0;
        uint32 ipQueuePassThrough = 1;


        if ((rtl8651_drvMaxFragSubPktCnt > 0) &&
                (qp->subPktCnt >= rtl8651_drvMaxFragSubPktCnt))
        {
                goto err_destroy_pkt_qp;
        }
        if ((rtl8651_drvMaxFragPoolCnt > 0) &&
                (frag_pkt_cnt >= rtl8651_drvMaxFragPoolCnt))
        {
                goto err_destroy_pkt_qp;
        }
        ...
        if (rtl8651a_dosFragmentCheck(pktHdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid, ip, pktHdrPtr) == 0)
        {
                goto err_dos_detected;
        }
        ...
        pktHdrPtr->ph_un_sw.ROMEDRV_USG._ph_dataLen = (ip->ip_len) - ihl;

        end = offset + pktHdrPtr->ph_un_sw.ROMEDRV_USG._ph_dataLen;


        if ((flags & 0x2000) == 0) {
                if ((end < qp->len) ||
                        ((qp->last_in & 0x01) && (end != qp->len)))
                {
                        goto err_TearDrop;
                }
...
# 1119 "rtl8651_ipQueue.c"
        prev = ((void *)0);
        if (qp->fragments == ((void *)0))
        {
                next = ((void *)0);
        }else
        {
                for(next = qp->fragments ; next ; next = next->ph_nextHdr)
                {
                        if (((uint16)(next->ph_un_sw.ROMEDRV_USG._ph_fragOffset)) >= offset)
                        {
                                break;
                        }
                        prev = next;
        ...
        if (prev) {
                int i = (((uint16)(prev->ph_un_sw.ROMEDRV_USG._ph_fragOffset)) + prev->ph_un_sw.ROMEDRV_USG._ph_dataLen) - offset;
        ...
        if (next && (((uint16)(next->ph_un_sw.ROMEDRV_USG._ph_fragOffset)) < end))
        {
                int i = end - ((uint16)(next->ph_un_sw.ROMEDRV_USG._ph_fragOffset));

                if (i < next->ph_un_sw.ROMEDRV_USG._ph_dataLen) {
                         goto err_TearDrop;
                }
        }

        pktHdrPtr->ph_un_sw.ROMEDRV_USG._ph_fragOffset=offset;




        pktHdrPtr->ph_nextHdr = next;
        if (prev)
        {
                prev->ph_nextHdr = pktHdrPtr;
        }else
        {
                qp->fragments = pktHdrPtr;
        }

        qp->meat += pktHdrPtr->ph_un_sw.ROMEDRV_USG._ph_dataLen;
        qp->subPktCnt ++;
        ip_frag_mem += pktHdrPtr->PKTHDRNXT.mbuf_first->m_extsize;
        frag_pkt_cnt ++;
        ...
        vlanp = &DrvTbl.vlan[pktHdrPtr->ph_vlanIdx & (8 - 1)];
        direction = (vlanp->internal?0:1);

        dosIgnore= (_rtl8651_dos_enable & _rtl8651_dosIgnoreTypeCheck(direction));


        ipQueuePassThrough = 1;
        if (dosIgnore == 1)
        {

                if ((qp->protocol == 17) && (_rtl8651_udpSizeThreshValue[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid] != 0xffff))
                        ipQueuePassThrough = 0;
        }else
        {
                ...
                if (_rtl8651_dos_sw[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid] & 0x00000040)
                        ipQueuePassThrough = 0;
                else if (qp->protocol == 17)
                {
                        if ((_rtl8651_udpSizeThreshValue[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid] != 0xffff) ||
                                        (_rtl8651_dos_sw[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid] & 0x00001000))
                                ipQueuePassThrough = 0;
                }
        }


        if (ipQueuePassThrough == 1)
        {
                if (qp->last_in & 0x02)
                {
                        uint32 pktComplete;
                        ...
                        frag_pkt_cnt -= qp->subPktCnt;
                        ip_frag_mem -= ((qp->fragments->PKTHDRNXT.mbuf_first->m_extsize) * (qp->subPktCnt));

                        retval = _rtl8651_flushFragQueue(qp->fragments, pktHdrPtr);
        ...
        } else
        {
                        ...
                        (qp->meat == qp->len))
                {
                        ...
                        if (qp->protocol == 17)
                        {
                                ...
                                if ((_rtl8651_udpSizeThreshValue[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid] != 0xffff) &&
                                        (qp->len > _rtl8651_udpSizeThreshValue[qp->fragments->ph_un_sw.ROMEDRV_USG._ph_dsid]))
                                {
                                        frag_freeMbufChain(qp);
                                        retval = 0;
                                        goto flow_done;
                                ...
                                {
                                        struct rtl_pktHdr *first = qp->fragments;
                                        struct ip *p;
                                        struct udphdr *u;

                                        p = (struct ip *)((uint32)(first->PKTHDRNXT.mbuf_first->m_data) + (uint32)(first->ph_iphdrOffset));
                                        u = (struct udphdr *)((uint32)p + (uint32)((p->ip_vhl & 0x0f) << 2));

                                        if (rtl8651a_dosFragUdpBombCheck(first->ph_un_sw.ROMEDRV_USG._ph_dsid, vlanp->vid, p, u, direction, qp->len, first) == 0)
                                        {
                                                frag_freeMbufChain(qp);
                                                retval = 0;
                                                goto flow_done;
...

        {
                ...
                }else
                {
                        ...
                        if (pktHdrPtr->ph_rxdesc >= 0)
                        {
                                ...
                                newHdr = newmb->m_pkthdr;
                                newCluster = newmb->m_extbuf;
                                newClusterId = newmb->m_extClusterId;
                                do {} while (0);
                                *newmb = *oldMbuf;
                                newmb->m_next = ((void *)0);
                                newmb->m_pkthdr = newHdr;

                                rxdesc = newHdr->ph_rxdesc;
                                *newHdr = *pktHdrPtr;
                                newHdr->PKTHDRNXT.mbuf_first = newmb;
                                newHdr->ph_rxdesc = rxdesc;
                                oldMbuf->m_extClusterId = newClusterId;
                                oldMbuf->m_data = newCluster;
                                oldMbuf->m_extbuf = newCluster;
                                oldMbuf->m_len = 0;
                                pktHdrPtr->ph_nextHdr = 0;
                                mBuf_freeMbufChain(pktHdrPtr->PKTHDRNXT.mbuf_first);


                                newHdr->ph_nextHdr = next;
                                if (prev)
                                        {
                                        prev->ph_nextHdr = newHdr;
                                }else
                                {
                                        qp->fragments = newHdr;
                                }
...
err_TearDrop:

        {
                ...
                {
                        pkthdr = qp->fragments;
                        pip = (struct ip *)((uint8 *)(pkthdr->PKTHDRNXT.mbuf_first->m_data) + (pkthdr->ph_iphdrOffset));
                }else
                {
                        pkthdr = pktHdrPtr;
                        pip = ip;
                ...
                rtl8651a_dosTearDropLogging(pkthdr->ph_un_sw.ROMEDRV_USG._ph_dsid, vlanp->vid, pip, direction, pkthdr);
        }
err_dos_detected:
err_destroy_pkt_qp:
        mBuf_freeMbufChain(pktHdrPtr->PKTHDRNXT.mbuf_first);
...
inline int32 _rtl8651_frag_NeedFragmentProcess(uint16 id, ipaddr_t sip, ipaddr_t dip, uint8 l4proto)
{
        ...
        hash = ipqhashfn(id, sip, dip, l4proto);
...
# 1497 "rtl8651_ipQueue.c"
int32 _rtl8651_ip_defrag(struct rtl_pktHdr * pkthdrPtr, struct ip *pip)
{
        struct ipq *qp;
        ...
        if ((qp = ip_frag_find(pip)) != ((void *)0)) {
                ...
                return ip_frag_queue(qp, pkthdrPtr, pip);
        }else
        {
...
static int32 _rtl8651_flushFragQueue(struct rtl_pktHdr *head, struct rtl_pktHdr *currentPkt)
{
        struct rtl_pktHdr *hp, *fp;
        int val;
        ...
        _rtl8651_fragQueueDropChain = 0;
        _rtl8651_fragQueueProcess = 1;

        _rtl8651_fragQueueNaptRedirect_MatchedIP = 0;
        ...
        {
                fp = hp->ph_nextHdr;
                hp->ph_nextHdr = ((void *)0);
                val = rtl8651_fwdEngineInput(hp);

                if (_rtl8651_fragQueueDropChain == 1)
                {
                        hp = fp;
                        goto errout;
                }
...
errout:
        ...
        while(hp)
        {
                fp = hp->ph_nextHdr;
                hp->ph_nextHdr = ((void *)0);
                mBuf_freeMbufChain(hp->PKTHDRNXT.mbuf_first);
                hp = fp;
        }
        _rtl8651_fragQueueProcess = 0;
        return -1;
...

        {
                ...
                while (hp)
                {
                        ...
                        if (fp == currentPkt)
                        {
                                if (hp == head)
                                {
                                        head->ph_nextHdr = currentPkt->ph_nextHdr;
                                        currentPkt->ph_nextHdr = head;
                                }else
                                {
                                        struct rtl_pktHdr *tmpHdr;

                                        hp->ph_nextHdr = head;

                                        tmpHdr = head->ph_nextHdr;
                                        head->ph_nextHdr = currentPkt->ph_nextHdr;
                                        currentPkt->ph_nextHdr = tmpHdr;
                                }
                                goto exchange_pkt;
                        }
                        hp = fp;
...
exchange_pkt:
                ...
                {
                        struct rtl_mBuf *mbuf_h;
                        struct rtl_mBuf *mbuf_c;
                        void *tmp_s;
                        uint32 tmp_u32;
                        uint16 tmp_u16;
                        int8 buff[sizeof(struct rtl_pktHdr)];
                        int16 rxdesc_h, rxdesc_c;
                        struct rtl_pktHdr *nextHdr_h, *nextHdr_c;


                        mbuf_h = head->PKTHDRNXT.mbuf_first;
                        mbuf_c = currentPkt->PKTHDRNXT.mbuf_first;


                        rxdesc_h = head->ph_rxdesc;
                        rxdesc_c = currentPkt->ph_rxdesc;
                        nextHdr_h = head->ph_nextHdr;
                        nextHdr_c = currentPkt->ph_nextHdr;

                        memset( (void *)buff, 0, sizeof(buff) );
                        rtl8651_memcpy((void *)buff, (void *)head, sizeof(struct rtl_pktHdr));
                        rtl8651_memcpy((void *)head, (void *)currentPkt, sizeof(struct rtl_pktHdr));
                        rtl8651_memcpy((void *)currentPkt, buff, sizeof(struct rtl_pktHdr));

                        head->PKTHDRNXT.mbuf_first = mbuf_h;
                        currentPkt->PKTHDRNXT.mbuf_first = mbuf_c;
                        head->ph_rxdesc = rxdesc_h;
                        currentPkt->ph_rxdesc = rxdesc_c;
                        head->ph_nextHdr = nextHdr_h;
                        currentPkt->ph_nextHdr = nextHdr_c;



                        tmp_s = mbuf_h->m_data;
                        mbuf_h->m_data = mbuf_c->m_data;
                        mbuf_c->m_data = tmp_s;

                        tmp_s = mbuf_h->m_extbuf;
                        mbuf_h->m_extbuf = mbuf_c->m_extbuf;
                        mbuf_c->m_extbuf = tmp_s;

                        tmp_u32 = mbuf_h->m_extClusterId;
                        mbuf_h->m_extClusterId = mbuf_c->m_extClusterId;
                        mbuf_c->m_extClusterId = tmp_u32;

                        tmp_u16 = mbuf_h->m_extsize;
                        mbuf_h->m_extsize = mbuf_c->m_extsize;
                        mbuf_c->m_extsize = tmp_u16;

                        tmp_u16 = mbuf_h->m_len;
                        mbuf_h->m_len = mbuf_c->m_len;
                        mbuf_c->m_len = tmp_u16;
                }
        }
...
concatenateOut:
        ...
        if ( _rtl8651_fragQueueNaptRedirect_MatchedIP )
        {
                struct rtl_pktHdr *m_pkt;
                struct ip *m_ip;
                ipaddr_t dip;
                ...
                while (m_pkt)
                {
                        m_ip = (struct ip *)(((uint32)(m_pkt->PKTHDRNXT.mbuf_first->m_data) + (m_pkt->ph_iphdrOffset)));


                        if ((((uint16)(m_ip->ip_off)) & 0x1fff))
                        {
                                if (!dip)
                                        dip = ((uint32) (*((uint8*)(&m_ip->ip_dst))<<24)|(*(((uint8*)(&m_ip->ip_dst))+1)<<16)|(*(((uint8*)(&m_ip->ip_dst))+2)<<8) | *(((uint8*)(&m_ip->ip_dst))+3));

                                do{ *((uint8*)(&m_ip->ip_dst)) = (uint8)((_rtl8651_fragQueueNaptRedirect_MatchedIP)>>24); *(((uint8*)(&m_ip->ip_dst))+1) = (uint8)((_rtl8651_fragQueueNaptRedirect_MatchedIP)>>16); *(((uint8*)(&m_ip->ip_dst))+2) = (uint8)((_rtl8651_fragQueueNaptRedirect_MatchedIP)>>8); *(((uint8*)(&m_ip->ip_dst))+3) = (uint8)((_rtl8651_fragQueueNaptRedirect_MatchedIP)); }while(0);
                                do { do { int32 accumulate = 0; if ((((_rtl8651_fragQueueNaptRedirect_MatchedIP)) != 0) && (((dip)) != 0)){ accumulate = (((dip)) & 0xffff); accumulate += (( ((dip)) >> 16 ) & 0xffff); accumulate -= (((_rtl8651_fragQueueNaptRedirect_MatchedIP)) & 0xffff); accumulate -= (( ((_rtl8651_fragQueueNaptRedirect_MatchedIP)) >> 16 ) & 0xffff); } if (((0) != 0) && ((0) != 0)){ accumulate += (0); accumulate -= (0); } do { accumulate += (((m_ip->ip_sum))); if (accumulate < 0) { accumulate = -accumulate; accumulate = (accumulate >> 16) + (accumulate & 0xffff); accumulate += accumulate >> 16; ((m_ip->ip_sum)) = ((uint16) ~accumulate); } else { accumulate = (accumulate >> 16) + (accumulate & 0xffff); accumulate += accumulate >> 16; ((m_ip->ip_sum)) = ((uint16) accumulate); } } while (0); }while(0); }while(0);
                        }

                        m_pkt = m_pkt->ph_nextHdr;
                }
        }
