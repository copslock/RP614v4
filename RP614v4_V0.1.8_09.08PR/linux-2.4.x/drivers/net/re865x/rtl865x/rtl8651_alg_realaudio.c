# 1 "rtl8651_alg_realaudio.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_alg_realaudio.c"
# 31 "rtl8651_alg_realaudio.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 32 "rtl8651_alg_realaudio.c" 2
# 1 "assert.h" 1
# 33 "rtl8651_alg_realaudio.c" 2
# 1 "types.h" 1
# 34 "rtl8651_alg_realaudio.c" 2
# 1 "rtl_queue.h" 1
# 35 "rtl8651_alg_realaudio.c" 2
# 1 "rtl_errno.h" 1
# 36 "rtl8651_alg_realaudio.c" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
# 43 "rtl8651_alg_realaudio.c" 2

int32 rtl8651_l4AliasHandleRealAudioClientOutbound(struct rtl_pktHdr *pkthdrPtr, struct ip *pip,
                        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *tb)
{
        struct tcphdr *tcp;
        int32 hlen, tlen, dlen,idx,port_idx;
        uint8 *data;
        uint16 version=0;
        uint16 msg_id=0;
        uint16 msg_len=0;

        if((alg_supports[pkthdrPtr->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00008000)==0) return 0;
        ...
        if(memcmp(data,"PNA",3)!=0) return 0;

        version=((data[3]<<8)&0xff00)|(data[4]&0xff);

        if(version>=256) return 0;

        idx=5;

        while(idx+4<dlen)
        {
                msg_id=((data[idx]<<8)&0xff00)|(data[idx+1]&0xff);
                msg_len=((data[idx+2]<<8)&0xff00)|(data[idx+3]&0xff);
                idx+=4;
                if(msg_id==0) return 0;
                port_idx=idx;
                idx+=msg_len;
                if(idx>dlen) return 0;
                if((msg_id==1)||(msg_id==7))
                {
                        uint16 udp_port;
                        udp_port=((data[port_idx]<<8)&0xff00)|(data[port_idx+1]&0xff);


                        if(udp_port!=0)
                        {
                                rtl8651_addUpnpMap(0x04, 0, 0,
                                        tb->insideGlobalIpAddr, udp_port,
                                        tb->insideLocalIpAddr, udp_port);
