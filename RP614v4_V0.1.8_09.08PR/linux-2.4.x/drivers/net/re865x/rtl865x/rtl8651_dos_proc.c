# 1 "rtl8651_dos_proc.c"
# 1 "<built-in>"
# 1 "<command line>"
# 1 "rtl8651_dos_proc.c"
# 11 "rtl8651_dos_proc.c"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 12 "rtl8651_dos_proc.c" 2
# 1 "assert.h" 1
# 13 "rtl8651_dos_proc.c" 2
# 1 "types.h" 1
# 14 "rtl8651_dos_proc.c" 2
# 1 "rtl_queue.h" 1
# 15 "rtl8651_dos_proc.c" 2
# 1 "rtl_errno.h" 1
# 16 "rtl8651_dos_proc.c" 2

# 1 "rtl8651_tblDrvFwdLocal.h" 1
# 169 "rtl8651_tblDrvFwdLocal.h"
# 1 "assert.h" 1
# 170 "rtl8651_tblDrvFwdLocal.h" 2
# 178 "rtl8651_tblDrvFwdLocal.h"
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
typedef struct _rtl8651_dosProc_blockIp_s {
        ipaddr_t ip;
        uint16 vid;
        uint32 dischargeTime;



        struct _rtl8651_dosProc_blockIp_s *next;
        struct _rtl8651_dosProc_blockIp_s **hdr;

        struct _rtl8651_dosProc_blockIp_s *jail_next;
} _rtl8651_dosProc_blockIp_t;

static uint8 _rtl8651_dosProc_sipBlock_sw[2] = {0};
static uint8 _rtl8651_dosProc_sipBlock_enable = 0;


static _rtl8651_dosProc_blockIp_t *sipBlockHash[16];

static uint32 _rtl8651_dos_sipBlock_sw[2] = {0};

static uint32 _rtl8651_dos_sipBlock_prisonTerm = 120;

_rtl8651_dosProc_blockIp_t *inuseJail = ((void *)0);
_rtl8651_dosProc_blockIp_t *emptyJail = ((void *)0);

static uint32 blockVlanIpCnt[8];

uint32 rtl8651_dosSIPBlockSw(uint32, uint32, uint32 *);

int32 _rtl8651_dosProc_blockSip_init(rtl8651_fwdEngineInitPara_t *);
void _rtl8651_dosProc_blockSip_reinit(void);
static __inline__ void _rtl8651_dosProc_blockSip(uint32, uint32, uint16, struct ip*);
static __inline__ void _rtl8651_dosProc_blockIP(uint32, uint32, uint16, ipaddr_t);

static __inline__ uint32 _rtl8651_dosProc_blockSip_hash(ipaddr_t);
static __inline__ void _rtl8651_dosProc_blockSip_imprison(uint16, ipaddr_t, uint32);
# 115 "rtl8651_dos_proc.c"
int32 _rtl8651_procDoSPkt_init(rtl8651_fwdEngineInitPara_t *para)
{
        int32 retval = 0;

        retval |= _rtl8651_dosProc_blockSip_init(para);

        return retval;
}
void _rtl8651_procDosPkt_reinit()
{
        _rtl8651_dosProc_blockSip_reinit();
}
void _rtl8651_procDoSPkt(uint32 item, uint32 dsid, uint16 vid, struct ip*ip, uint8 direction, struct rtl_pktHdr* pktHdr)
{
        _rtl8651_dosProc_blockSip(item, dsid, vid, ip);
}
void _rtl8651_procDoSIP(uint32 item, uint32 dsid, uint16 vid, ipaddr_t ip)
{
        _rtl8651_dosProc_blockIP(item, dsid, vid, ip);
}



int32 _rtl8651_dosProc_blockSip_init(rtl8651_fwdEngineInitPara_t *para)
{
        int32 idx;
        uint32 JailCnt;
        _rtl8651_dosProc_blockIp_t *ptr;


        for (idx = 0 ; idx < 2 ; idx ++)
                _rtl8651_dosProc_sipBlock_sw[idx] = 0;

        if (!para)
                goto errout;

        JailCnt = para->dosProc_sipblock_count;

        if (JailCnt == 0)
                goto disableOut;


        emptyJail = (_rtl8651_dosProc_blockIp_t *)rtlglue_malloc(JailCnt*sizeof(_rtl8651_dosProc_blockIp_t));
        if (!emptyJail)
                goto errout;
        ...
        memset(emptyJail, 0, (JailCnt*sizeof(_rtl8651_dosProc_blockIp_t)));


        ptr = emptyJail;
        for (idx = 0 ; idx < JailCnt ; idx ++, ptr ++)
        {
                if (idx != (JailCnt - 1))
                {
                        ptr->jail_next = ptr + 1;
        ...
        for (idx = 0 ; idx < 16 ; idx ++)
                sipBlockHash[idx] = ((void *)0);


        for (idx = 0 ; idx < 8 ; idx ++)
                blockVlanIpCnt[idx] = 0;


        for (idx = 0 ; idx < 2 ; idx ++)
                _rtl8651_dos_sipBlock_sw[idx] = ( 0x00000010 | 0x00000020 | 0x00000040 | 0x00001000 | 0x00040000 | 0x00002000 | 0x00004000 | 0x00008000 | 0x00010000 | 0x00020000 | 0x00100000 | 0x00200000 | 0x00400000 | 0x00080000);

        _rtl8651_dosProc_sipBlock_enable = 1;
        return 0;
errout:
        _rtl8651_dosProc_sipBlock_enable = 0;
        return -1;
disableOut:
        _rtl8651_dosProc_sipBlock_enable = 0;
        return 0;
}
void _rtl8651_dosProc_blockSip_reinit()
{
        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return; } while (0);;


        rtl8651_dosProc_blockSip_freeAll();
}

static __inline__ uint32 _rtl8651_dosProc_blockSip_hash(ipaddr_t ip)
{
        return (ip & (16 -1));
}
...
{
        _rtl8651_dosProc_blockIp_t *ptr;
        uint32 hash = _rtl8651_dosProc_blockSip_hash(sip);

        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return -1 ; } while (0);;

        ptr = sipBlockHash[hash];

        while (ptr)
        {
                if ((ptr->ip == sip) && (ptr->vid == vid))
                {
                        if (ptr->dischargeTime < _sysUpSeconds)
                        {
...
static __inline__ void _rtl8651_dosProc_blockSip_imprison(uint16 vid, ipaddr_t sip, uint32 prisonTerm)
{
        _rtl8651_dosProc_blockIp_t *prisoner;
        _rtl8651_dosProc_blockIp_t *ptr, *prev;
        uint32 hash = _rtl8651_dosProc_blockSip_hash(sip);
        ...
        if (!emptyJail)
                return;
        prisoner = emptyJail;
        emptyJail = emptyJail->jail_next;



        _rtl8651_removeNaptConnectionByIP(sip);
        ...
        prisoner->ip = sip;
        prisoner->vid = vid;
        prisoner->dischargeTime = _sysUpSeconds + prisonTerm;
        ...
        ptr = inuseJail;
        prev = ((void *)0);
        while (ptr && ((ptr->dischargeTime) < (prisoner->dischargeTime)))
        {
                prev = ptr;
                ptr = ptr->jail_next;
        }

        prisoner->jail_next = ptr;
        if (prev)
        {
                prev->jail_next = prisoner;
        }else
        {
                inuseJail = prisoner;
        }

        prisoner->next = sipBlockHash[hash];
        prisoner->hdr = &sipBlockHash[hash];
        sipBlockHash[hash] = prisoner;




        blockVlanIpCnt[vid&(8 -1)] ++;


        _rtl8651_arrangeAllACL();
}

void _rtl8651_dosProc_blockSip_discharge(uint32 currentTime)
{
        _rtl8651_dosProc_blockIp_t *ptr, *prev, *hashptr;
        int32 AclArrange = 0;


        while (inuseJail && (inuseJail->dischargeTime < currentTime))
        {
                AclArrange = 1;
                ...
                blockVlanIpCnt[(inuseJail->vid)&(8 -1)] --;


                ptr = inuseJail;
                inuseJail = inuseJail->jail_next;
...
static __inline__ void _rtl8651_dosProc_blockSip(uint32 item, uint32 dsid, uint16 vid, struct ip*ip)
{

        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return; } while (0);;
        do { if (_rtl8651_dosProc_sipBlock_sw[dsid] == 0) return; } while (0);;

        if (item & _rtl8651_dos_sipBlock_sw[dsid])
        {
                ipaddr_t sip = ((uint32) (*((uint8*)(&ip->ip_src))<<24)|(*(((uint8*)(&ip->ip_src))+1)<<16)|(*(((uint8*)(&ip->ip_src))+2)<<8) | *(((uint8*)(&ip->ip_src))+3));

                _rtl8651_dosProc_blockSip_imprison(vid, sip, _rtl8651_dos_sipBlock_prisonTerm);
        }
}

static __inline__ void _rtl8651_dosProc_blockIP(uint32 item, uint32 dsid, uint16 vid, ipaddr_t ip)
{
        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return; } while (0);;
        ...
        if ( dsid == 2 )
        {
                ...
                for ( idx = 0 ; idx < 2 ; idx ++ )
                {
                        if (_rtl8651_dosProc_sipBlock_sw[idx] == 0) { continue; };

                        if (item & _rtl8651_dos_sipBlock_sw[idx])
                        {
                                goto imprison;
                        }
                }
...
imprison:
        _rtl8651_dosProc_blockSip_imprison(vid, ip, _rtl8651_dos_sipBlock_prisonTerm);
}
# 410 "rtl8651_dos_proc.c"
uint32 _rtl8651_addAclRuleForSipBlock(uint16 vid, uint32 aclStart, uint32 aclEnd, int8 *enoughFlag)
{
        _rtl8651_dosProc_blockIp_t *ptr;
        ...
        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return 0 ; } while (0);;

        if (blockVlanIpCnt[vid&(8 -1)] == 0)
                return 0;
        ...
        while (ptr)
        {
                if (ptr->vid == vid)
                {
                        ...
                        { (&rule)->un_ty.L3L4._srcIpAddr = ptr->ip; (&rule)->un_ty.L3L4._srcIpAddrMask = 0xffffffff; (&rule)->un_ty.L3L4._dstIpAddr = 0; (&rule)->un_ty.L3L4._dstIpAddrMask = 0; (&rule)->un_ty.L3L4._tos = 0; (&rule)->un_ty.L3L4._tosMask = 0; (&rule)->un_ty.L3L4.is.ip._proto = 0; (&rule)->un_ty.L3L4.is.ip._protoMask = 0; (&rule)->un_ty.L3L4.is.ip.un._flag = 0; (&rule)->un_ty.L3L4.is.ip._flagMask = 0; (&rule)->ruleType_ = 0x01; (&rule)->actionType_ = 0x02;};
...
uint32 rtl8651_dosSIPBlockSw(uint32 item, uint32 dsid, uint32 *flag)
{
        ...
        if (!flag)
        {
                ...
                return (item & _rtl8651_dos_sipBlock_sw[dsid]);
        }else
        {
                if (*flag)
                {
                        _rtl8651_dos_sipBlock_sw[dsid] = (_rtl8651_dos_sipBlock_sw[dsid] | item);
                }else
                {
                        _rtl8651_dos_sipBlock_sw[dsid] = (_rtl8651_dos_sipBlock_sw[dsid] & ~item);
                }
...
void rtl8651_dosProc_blockSip_freeAll(void)
{
        ...
        _rtl8651_dosProc_blockSip_discharge(0xffffffff);
        do {} while (0);
        for (idx = 0 ; idx < 8 ; idx ++)
        {
                do {} while (0);
...
int32 rtl8651_dosProc_blockSip_setPrisonTime(uint32 prisonTime)
{
        do { if (_rtl8651_dosProc_sipBlock_enable == 0) return -1 ; } while (0);;
        rtlglue_drvMutexLock();
        _rtl8651_dos_sipBlock_prisonTerm = prisonTime;
        rtlglue_drvMutexUnlock();
...
void rtl8651_dosProc_blockSip_enable(uint32 dsid, uint32 enable)
{
        ...
        if (enable)
        {
                _rtl8651_dosProc_sipBlock_sw[dsid] = 1;
        ...
        {
                if (_rtl8651_dos_sipBlock_sw[dsid])
                {
                        rtl8651_dosProc_blockSip_freeAll();
                }
                _rtl8651_dosProc_sipBlock_sw[dsid] = 0;
...
{
        _rtl8651_dosProc_blockIp_t *ptr;
        int32 inuseJailCnt, emptyJailCnt, idx;
        ...
        printk("\tPrison Time:\t\t%d sec.\n\n", _rtl8651_dos_sipBlock_prisonTerm);
        printk("DOS attack Mask:\n");
        printk("\tSession\t\tMask\n");
        printk("\t========================\n");
        for (idx = 0 ; idx < 2 ; idx ++)
                printk("\t%d\t\t0x%X\n", idx, _rtl8651_dos_sipBlock_sw[idx]);
        ...
        printk("\tJail\t%d\t%d\t%d\n", (inuseJailCnt + emptyJailCnt), inuseJailCnt, emptyJailCnt);

        printk("\n");

        printk("Blocked IP List:\n\n");
        printk("\t\tIP\t\tVID\tDischarge Time(sec.)\n");
        printk("\t==========================================================\n");
        ...
        {
                printk("\t%03u.%03u.%03u.%03u\t\t0x%X\t\t%d\n", ((unsigned char *)&ptr->ip)[0], ((unsigned char *)&ptr->ip)[1], ((unsigned char *)&ptr->ip)[2], ((unsigned char *)&ptr->ip)[3], ptr->vid, ptr->dischargeTime);
                ptr = ptr->jail_next;
        }
        printk("\t==========================================================\n");
        printk("\t\t\t\t\tCurrent Time: %d sec.\n", _sysUpSeconds);
        rtlglue_drvMutexUnlock();
}
