...
struct rtl_pktHdr
{
	...
	uint16    ph_flags;				   /*  NEW:Packet header status bits */
		...
		#define PKTHDR_QUEUE_ID		0x3800
#endif
		//#define PKT_INCOMING		0x1000	   /* Incoming: packet is incoming (as seen by protocol stack)*/
		//#define PKT_OUTGOING		0x0800	   /*  Outgoing: packet is outgoing (as seen by protocol stack) */
	...
	int16 ph_rxdesc;
		//ph_rxdesc:  =-1: If pkt generated from protocol stack,  
		//			=totalRxPkthdr: If pkt generated from forwarding engine
		//			0~totalRxPkthdr-1: If pkt is received from NIC.
	...
	*/
	...
	union {
		struct {
			...
			uint8	_ph_dsid;
			uint16	_ph_sport;
			uint8	_ph_priority;	/* This field has exceeded the original size of packet header (32 byte). */
			uint8	_ph_procFlag;	/* This flag is used to indicate what process is done for this packet */
			uint8	_ph_reserved1;	/* The following reserved fields are for 4-byte aligned. */
			uint8	_ph_reserved2;
		} ROMEDRV_USG;
		union {
	...
	#define	ph_priority			ph_un_sw.ROMEDRV_USG._ph_priority
 ...
 */
void mBuf_getBMjmpTable(uint8 *pat,  uint16 *jump_tbl,uint16 patLen);
int32 mBuf_BMpatternMatch(struct rtl_mBuf *m, uint32 len, uint8 *delimiter, uint32 delimitLen, uint16 *jmp_tbl);
