...
# 1 "<command line>"
# 1 "rtl8651_alg_ftp.c"
# 119 "rtl8651_alg_ftp.c"
# 1 "rtl8651_alg_ftp.h" 1
# 58 "rtl8651_alg_ftp.h"
# 1 "rtl_types.h" 1
# 204 "rtl_types.h"
...
# 59 "rtl8651_alg_ftp.h" 2
# 1 "assert.h" 1
# 60 "rtl8651_alg_ftp.h" 2
# 1 "types.h" 1
# 61 "rtl8651_alg_ftp.h" 2
# 1 "rtl_queue.h" 1
# 62 "rtl8651_alg_ftp.h" 2
# 1 "rtl_errno.h" 1
# 63 "rtl8651_alg_ftp.h" 2
# 1 "mbuf.h" 1
# 30 "mbuf.h"
struct rtl_mBuf
{
...
# 120 "rtl8651_alg_ftp.c" 2




static uint8 *_rtl8651_l4_ftp_alias_buf[2];
 ...
 struct rcvdelimit {
        ...
        uint32 length;
        uint8 delimiter[8];
        uint16 jump_table[256];
 } PASVpattern;

enum _rtl8651_l4FtpMessageType
{
        _RTL8651L4_FTP_PORT_COMMAND,
        _RTL8651_L4_FTP_EPRT_COMMAND,
        _RTL8651_L4_FTP_227_REPLY,
        _RTL8651_L4_FTP_229_REPLY,
        _RTL8651_L4_FTP_UNKNOWN_MESSAGE
};


static ipaddr_t _rtl8651_true_addr;
static uint16 _rtl8651_true_port;
# 158 "rtl8651_alg_ftp.c"
static int32 _rtl8651_l4ParseFtpPortCommand(int8 * sptr, int32 dlen)
{
        int8 ch;
        int32 i, state;
        uint32 addr;
...
# 173 "rtl8651_alg_ftp.c"
        ...
        if (state == 13)
        {
                _rtl8651_true_addr = addr;
                _rtl8651_true_port = port;
                return 1;
        }
        else
                return 0;
}
# 276 "rtl8651_alg_ftp.c"
static int32 _rtl8651_l4ParseFtpEprtCommand(int8 * sptr, int32 dlen)
{
        int8 ch, delim;
        int32 i, state;
        uint32 addr;
...
# 412 "rtl8651_alg_ftp.c"
static int32 _rtl8651_l4ParseFtp227Reply(int8 * sptr, int32 dlen)
{
        int8 ch;
        int32 i, state;
        uint32 addr;
...
# 524 "rtl8651_alg_ftp.c"
static int32 _rtl8651_l4ParseFtp229Reply(int8 * sptr, int32 dlen)
{
        int8 ch, delim;
        int32 i, state;
        uint16 port;
# 537 "rtl8651_alg_ftp.c"
        if (dlen < 11)
                return 0;
...
# 629 "rtl8651_alg_ftp.c"
static int32
_rtl8651_l4NewFtpClientMessage(struct ip *pip,
                          rtl8651_tblDrv_naptTcpUdpFlowEntry_t *tb,
                          int32 _rtl8651_l4FtpMessageType)
{
        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s entry;
        struct rtl8651_tblDrv_naptTcpUdpFlowEntry_s *ftp_tb=&entry;
        int32 diff_len = 0;
        struct tcphdr *tc;
        ipaddr_t ip_s, ip_d, ip_alias = 0;
        uint16 ip_dp, ip_aliasp = 0;

        tc = (struct tcphdr *) ((int8 *) pip + ((*(uint8*)pip&0xf) << 2));

        ip_s = ((uint32) (*((uint8*)(&pip->ip_src))<<24)|(*(((uint8*)(&pip->ip_src))+1)<<16)|(*(((uint8*)(&pip->ip_src))+2)<<8) | *(((uint8*)(&pip->ip_src))+3));
        ip_d = ((uint32) (*((uint8*)(&pip->ip_dst))<<24)|(*(((uint8*)(&pip->ip_dst))+1)<<16)|(*(((uint8*)(&pip->ip_dst))+2)<<8) | *(((uint8*)(&pip->ip_dst))+3));
        ip_dp = tc->th_dport;


        if (_rtl8651_true_port < 1024){
                return 0;
        }
        ...
        if(tb->natlink){
                ...
                ip_aliasp = _rtl8651_true_port;
                goto nat_proc;
        }


        ftp_tb=&entry;
        if(_rtl8651_getOutboundNaptFlow(1,_rtl8651_true_addr,_rtl8651_true_port,ip_d,ip_dp-1, &ftp_tb)==0){
                ip_alias = ftp_tb->insideGlobalIpAddr;
                ip_aliasp = ftp_tb->insideGlobalPort;
        }
        else
        {
                int r;

                _rtl8651_softNaptPolicyRoute(0, ip_s,ip_d,ip_dp,&ip_alias,&ip_aliasp);
                if (!ip_alias)
                {

                        if( _rtl8651_getUsableExtIpAndPort(1,
                                _rtl8651_true_addr,_rtl8651_true_port,ip_d,ip_dp-1,&ip_alias,&ip_aliasp) != 0 ){
                                return 0;
                        }
                }
                do {} while (0);
                ...
                _rtl8651_addAlgQosUpnpMap(0x02|0x04|0x0080, ip_d, 0,
                                ip_alias, ip_aliasp,_rtl8651_true_addr,_rtl8651_true_port, 3 );


                r=_rtl8651_addAlgQosNaptConnection(1, 1|0x0080,1, _rtl8651_true_addr, _rtl8651_true_port, &ip_alias, &ip_aliasp, ip_d, ip_dp-1,3);

                ftp_tb=&entry;

                _rtl8651_getOutboundNaptFlow(1,_rtl8651_true_addr,_rtl8651_true_port,ip_d,ip_dp-1, &ftp_tb);
                if (ftp_tb != ((void *)0)){
                        ftp_tb->tcpAckModified = 0;
                        ftp_tb->tcpFtpLastLineCRLFTermed = 0;
                        ftp_tb->tcpStateINdex = 0;
...
nat_proc:
        ...
        if (ftp_tb != ((void *)0))
        {
...

                {
                        ...
                        int32 a1, a2, a3, a4, p1, p2;


                        stemp = (int8*)_rtl8651_l4_ftp_alias_buf[0];
# 736 "rtl8651_alg_ftp.c"
                        a1 = (ip_alias >> 24)&0xff;
                        a2 = (ip_alias >> 16)&0xff;
                        a3 = (ip_alias >> 8)&0xff;
                        a4 = ip_alias & 0xff;
                        ...
                        switch (_rtl8651_l4FtpMessageType)
                        {
                                case _RTL8651L4_FTP_PORT_COMMAND:
                                        ...
                                        if (_rtl8651_l4FtpMessageType == _RTL8651L4_FTP_PORT_COMMAND)
                                        {
                                        ...
                                        break;
                                case _RTL8651_L4_FTP_EPRT_COMMAND:

                                        sprintf(stemp, "EPRT |1|%d.%d.%d.%d|%d|\r\n",
                                                        a1, a2, a3, a4, ip_aliasp);
                                        break;
                        }
...

                {
                        ...
                        _rtl8651_l4AddSeq(pip, tb, delta + slen - dlen);
                        diff_len = slen - dlen;
                }
...
# 818 "rtl8651_alg_ftp.c"
static int32
_rtl8651_l4NewFtpServerMessage(struct ip *pip,
                          rtl8651_tblDrv_naptTcpUdpFlowEntry_t *tb,
                          int32 _rtl8651_l4FtpMessageType)
{
        int32 diff_len = 0;
        ipaddr_t ip_s,ip_d = 0;
        struct tcphdr *tc;
...

        {
                ...
                _rtl8651_addAlgQosUpnpMap(0x02|0x04|0x0080, ip_d, 0,
                                tb->insideGlobalIpAddr, _rtl8651_true_port,
                                ip_s, _rtl8651_true_port,3);
...
nat_proc:

                {
                        ...
                        stemp = (int8*)_rtl8651_l4_ftp_alias_buf[0];
# 869 "rtl8651_alg_ftp.c"
                        a1 = (tb->insideGlobalIpAddr >> 24) & 0xff;
                        a2 = (tb->insideGlobalIpAddr >> 16) & 0xff;
                        a3 = (tb->insideGlobalIpAddr >> 8) & 0xff;
                        a4 = (tb->insideGlobalIpAddr) & 0xff;


                        switch (_rtl8651_l4FtpMessageType)
                        {
                                case _RTL8651_L4_FTP_227_REPLY:
                                        ...
                                        p1 = (_rtl8651_true_port>>8)&0xff;
                                        p2 = (_rtl8651_true_port)&0xff;
                                ...
                                case _RTL8651_L4_FTP_EPRT_COMMAND:
                                        ...
                                        sprintf(stemp, "EPRT |1|%d.%d.%d.%d|%d|\r\n",
                                                        a1, a2, a3, a4, _rtl8651_true_port);
                                        break;
                                case _RTL8651_L4_FTP_229_REPLY:
...
static int32 init_ftp = 0;
int32 rtl8651_l4AliasFtpInit(void *arg){



        int8 pasvCmdStr[]="PASV\r\n";
        ...
        _rtl8651_l4_ftp_alias_buf[0] = (uint8 *)rtlglue_malloc(576 +64);
        _rtl8651_l4_ftp_alias_buf[1] = (uint8 *)rtlglue_malloc(576 +64);
        do {} while (0);
        do {} while (0);


        rtl8651_memcpy((void*)&PASVpattern.delimiter[0], pasvCmdStr, sizeof(pasvCmdStr)+1);
        PASVpattern.length=strlen(pasvCmdStr);
        mBuf_getBMjmpTable( (uint8 *)pasvCmdStr, (uint16 *)&PASVpattern.jump_table[0], PASVpattern.length, 1 );
        return 0;
}
...
void rtl8651_l4AliasFtpReInit(void){
        ...
        _rtl8651_true_addr=0;
        _rtl8651_true_port=0;
        memset(&PASVpattern,0,sizeof(PASVpattern));
}
# 990 "rtl8651_alg_ftp.c"
int32 rtl8651_l4AliasHandleFtpCleintOutbound(
        struct rtl_pktHdr *indat,
        struct ip *inhdr,
...
)
{
        ...
        struct ip *pip;
        int32 off_set;
        struct ip *inip = (struct ip*)(inhdr);
        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *tb;

        if((alg_supports[indat->ph_un_sw.ROMEDRV_USG._ph_dsid]&0x00000008)==0) return 0;

        tb = (rtl8651_tblDrv_naptTcpUdpFlowEntry_t *)link;


        pkthdrPtr = (struct rtl_pktHdr * )indat;
        m = pkthdrPtr->PKTHDRNXT.mbuf_first;

        off_set = ((uint8 *)inip) - m->m_data;

        if(off_set < 0 || off_set > 40 || tb == ((void *)0)){
                return -1;
        }


        if (m->m_pkthdr->ph_len > 576)
        {
                return -1;
        }
        ...
        newpacket = (int8 *)_rtl8651_l4_ftp_alias_buf[1];

        mBuf_copyToUserBuffer(m, off_set, m->m_pkthdr->ph_len - off_set, newpacket);

        do {} while (0);
        pip = (struct ip *) newpacket;
        ...
        if (dlen <= 576 && (tb->tcpFtpLastLineCRLFTermed))
        {
                _rtl8651_l4FtpMessageType = _RTL8651_L4_FTP_UNKNOWN_MESSAGE;


                if (_rtl8651_l4ParseFtpPortCommand(sptr, dlen))
                        _rtl8651_l4FtpMessageType = _RTL8651L4_FTP_PORT_COMMAND;
                else if (_rtl8651_l4ParseFtpEprtCommand(sptr, dlen))
                        _rtl8651_l4FtpMessageType = _RTL8651_L4_FTP_EPRT_COMMAND;

                if (_rtl8651_l4FtpMessageType != _RTL8651_L4_FTP_UNKNOWN_MESSAGE)
                        _rtl8651_l4NewFtpClientMessage(pip, tb, _rtl8651_l4FtpMessageType);
        }
        ...
        if (dlen)
        {
                ...
                if((sptr[tlen - 2] == '\r') && (sptr[tlen - 1] == '\n'))
                        link->tcpFtpLastLineCRLFTermed = 1;
                else
                        link->tcpFtpLastLineCRLFTermed = 0;
                ...
                mBuf_copyToMbuf(m, off_set, tlen, newpacket);
                do {} while (0);
        }
...
int32 rtl8651_l4AliasHandleFtpCleintInbound(
        struct rtl_pktHdr *indat,
        struct ip *inhdr,
        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link
)
...
# 1123 "rtl8651_alg_ftp.c"
int32 rtl8651_l4AliasFilterFtpServerInboundPasvCmd(
        struct rtl_pktHdr *indat,
        struct ip *inhdr,
        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link
)
{
        ...
        if(dlen)
                pos=mBuf_BMpatternMatch(m, m->m_len, (uint8*)&PASVpattern.delimiter,PASVpattern.length, (uint16 *)&PASVpattern.jump_table, 1);
...
# 1211 "rtl8651_alg_ftp.c"
int32 rtl8651_l4AliasHandleFtpServerOutbound(
        struct rtl_pktHdr *indat,
        struct ip *inhdr,
        rtl8651_tblDrv_naptTcpUdpFlowEntry_t *link
)
{
        ...
        if (dlen <= 576 && (tb->tcpFtpLastLineCRLFTermed) )
        {
                ...
                if (_rtl8651_l4ParseFtp227Reply(sptr, dlen))
                        _rtl8651_l4FtpMessageType = _RTL8651_L4_FTP_227_REPLY;
                else if (_rtl8651_l4ParseFtp229Reply(sptr, dlen))
                        _rtl8651_l4FtpMessageType = _RTL8651_L4_FTP_229_REPLY;

                if (_rtl8651_l4FtpMessageType != _RTL8651_L4_FTP_UNKNOWN_MESSAGE)
                        _rtl8651_l4NewFtpServerMessage(pip, tb, _rtl8651_l4FtpMessageType);
        }
