...
#ifdef CONFIG_RTL865XB_3G
{
			...
			if((unsigned int)skb->data-(unsigned int)skb->head>=14)
			{
				...
				// virtual gateway's MAC address (eth0's mac + 2)
				rtl865x_getWanMac(&skb->data[6]);
				skb->data[11]+=2;
				
				skb->data[12]=0x08; //data[13] already memset to 0 above.
				skb->data[13]=0;
				skb->len+=14;
			}
...
//			printk("XXX Can't register wan device\n");
		...
		if(0!=devglue_regExtDevice(dev->name, 8, CONFIG_RTL865XB_3G_PORT, &myLinkID))
			printk("XXX Can't register a link ID for device %s!!!\n", dev->name );
		else
			printk("Device %s on vlan ID %d using Link ID %d.\n", dev->name,8,  myLinkID);
	}
#endif
