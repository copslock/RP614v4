...
#ifdef CONFIG_RTL865X
#define RTL8139CP_KFREE_SKB(skb)	dev_kfree_skb_any(skb)
#else
#define RTL8139CP_KFREE_SKB(skb)	dev_kfree_skb_irq(skb)
#endif
...
static void  cp_rx (struct cp_private *cp)
{
	...
	while (rx_work--) {
...

		{
			...
			retval=rtlglue_extPortRecv(skb, skb->data, skb->len, CONFIG_8139CP_VID, 1<<cp->rtl8650extPortNum, cp->rtl8650linkNum);
			if(retval==SUCCESS){
				//8651 fwd engine consumed the packet.
			}else if (retval==FAILED){
				//exception. Drop it.
				RTL8139CP_KFREE_SKB(skb);
			}else{
