 ...
 *
 *	Version:	$Id: skbuff.c,v 1.8 2005/06/03 06:33:30 chenyl Exp $
 *
 *	Fixes:	
...
/*
	chenyl (2005/06/03):
		In original LINUX skbuff model, system never actually free skb in irq().

		In irq process, system only put the complete skbuffs to 'softnet_data[cpu].completion_queue' simply
		and schedule a software IRQ (NET_TX_SOFTIRQ, net_tx_action() in dev.c) to free these skbuffs.

		However, when system under heavy load,
		this software IRQ NET_TX_SOFTIRQ would be 'live locked',
		it would be preempted by HW interrupts and can not be processed.
		And skbuffs would not be freed, system resource would be exhausted.

		Therefore, we would modify LINUX's mode to release skbuff immediately
		even it in IRQ.
*/
#if 0	/* we don't print warning message here */
		if (in_irq()) {
