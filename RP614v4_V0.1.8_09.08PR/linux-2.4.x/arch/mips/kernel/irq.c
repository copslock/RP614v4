 ...
 * prefer.
 */
__IRAM_GEN int handle_IRQ_event(unsigned int irq, struct pt_regs * regs, struct irqaction * action)
{
	int status;
	int cpu = smp_processor_id();
 ...
 * handlers).
 */
__IRAM_GEN asmlinkage unsigned int do_IRQ(int irq, struct pt_regs *regs)
{
	/*
		...
		void *dev_id)
{
	...
	if (retval)
		kfree(action);
	printk("===> Request IRQ %d for %s, ret=%d\n", irq, devname, retval);
	return retval;
}
